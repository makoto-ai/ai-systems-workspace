...
class SmartDevelopmentAutomation:
    ...
    def is_safe_operation(self, operation_type: str, target_file: str) -> bool:
        ...
        dangerous_patterns = [
            "delete", "remove", "rm", "DROP", "DELETE FROM",
            "truncate", "format", "wipe"
        ]
        ...
        # 危険なパターンを含む
        if any(pattern in operation_type.lower() or pattern in target_file.lower() 
    ...
    def run_development_automation(self):
        ...
        if self.development_phase == "ACTIVE_DEV":
            automation_tasks.extend([
                {"name": "auto_test", "description": "自動テスト実行", "safe": True, "cpu_impact": "MEDIUM"},
                {"name": "auto_lint", "description": "コード品質チェック", "safe": True, "cpu_impact": "LOW"},
                {"name": "auto_security_scan", "description": "セキュリティスキャン", "safe": True, "cpu_impact": "MEDIUM"}
            ])
        ...
        if current_cpu > max_cpu:
            ...
            automation_tasks = [task for task in automation_tasks if task["cpu_impact"] == "LOW"]
    ...
    def start_background_automation(self):
        ...
        def automation_loop():
            while True:
                try:
                    # 負荷チェック
                    if psutil.cpu_percent() < self.config["performance_optimization"]["max_cpu_usage"]:
