...
class LearningPredictionEngine:
    ...
    def collect_historical_data(self) -> Dict[str, Any]:
        ...
        print(f"📊 データ収集完了: {len(historical_data['security_reports'])}件のレポート")
    ...
    def learn_problem_patterns(self, historical_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 学習結果保存
        patterns_file = self.learning_dir / f"learned_patterns_{datetime.datetime.now().strftime('%Y%m%d')}.json"
        with open(patterns_file, 'w', encoding='utf-8') as f:
            # Counter オブジェクトを辞書に変換
            serializable_patterns = {
                'frequent_threats': dict(patterns['frequent_threats']),
                'error_sequences': patterns['error_sequences'],
                'time_patterns': {k: list(v) for k, v in patterns['time_patterns'].items()},
                'file_patterns': {k: list(v) for k, v in patterns['file_patterns'].items()},
    ...
    def predict_future_threats(self, patterns: Dict[str, Any], current_state: Dict[str, Any]) -> Dict[str, Any]:
        """将来の脅威予測"""
        print("🔮 将来脅威予測開始...")
        ...
        # 予防アクション生成
        predictions['preventive_actions'] = self._generate_preventive_actions(predictions)
        
        # 予測結果保存
        prediction_file = self.learning_dir / f"threat_predictions_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        with open(prediction_file, 'w', encoding='utf-8') as f:
            json.dump(predictions, f, ensure_ascii=False, indent=2)
        
        print(f"🔮 脅威予測完了: {len(predictions['high_risk_threats'])}件の高リスク脅威を検出")
        return predictions
    
    def generate_improvement_suggestions(self, patterns: Dict[str, Any], predictions: Dict[str, Any]) -> Dict[str, Any]:
        """自動改善提案生成"""
        print("💡 改善提案生成開始...")
        ...
        high_activity_hours = [hour for hour, events in time_patterns.items() if len(events) > 5]
        ...
        # 提案保存
        suggestions_file = self.learning_dir / f"improvement_suggestions_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def run_learning_cycle(self) -> Dict[str, Any]:
        ...
        # レポート保存
        report_file = self.learning_dir / f"learning_cycle_report_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        with open(report_file, 'w', encoding='utf-8') as f:
            # Counter オブジェクトの処理
            if 'frequent_threats' in learning_report['patterns']:
                learning_report['patterns']['frequent_threats'] = dict(learning_report['patterns']['frequent_threats'])
