...
class AdvancedThreatAnalyzer:
                    ...
                    for match in matches:
                        ...
                        if line_num <= len(lines):
                            ...
                            if (line_content.startswith('#') or 
                                '# hashlib.md5()' in line_content or
                                '# hashlib.sha1()' in line_content or
                                'advanced_threat_analyzer.py' in str(py_file)):
                                continue
    ...
    def generate_security_report(self, vulnerabilities: Dict, dependencies: Dict) -> Dict[str, Any]:
        """セキュリティレポート生成"""
    ...
    def _generate_recommendations(self, vulnerabilities: Dict, dependencies: Dict) -> List[str]:
        """修復推奨事項生成"""
        recommendations = []
        ...
        if vulnerabilities['command_injection']:
            recommendations.append("Validate and sanitize all user inputs before command execution")
        
        if vulnerabilities['weak_crypto']:
            recommendations.append("Replace weak cryptographic algorithms with stronger alternatives (SHA-256, AES)")
        ...
        if dependencies['insecure_versions']:
            recommendations.append("Pin package versions in requirements.txt for reproducible builds")
        
        if dependencies['outdated_packages']:
            recommendations.append("Regularly update dependencies and review security advisories")
    ...
    def run_comprehensive_analysis(self) -> Dict[str, Any]:
        ...
        # 結果保存
        report_file = self.project_root / "data" / f"advanced_security_report_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
