...
class AutoRepairSystem:
    ...
    def repair_file_permissions(self, file_path: str, target_permission: str = "644") -> Dict:
        """ファイル権限修復"""
        repair_action = {
        ...
        try:
            ...
            if os.path.exists(file_path):
                ...
                # 権限修復実行
                result = subprocess.run(['chmod', target_permission, file_path], 
                                      capture_output=True, text=True)
                
                if result.returncode == 0:
                    repair_action['success'] = True
                    repair_action['details'] = f'ファイル権限を{target_permission}に修復'
        ...
        try:
            if os.path.exists(file_path):
                ...
                if file_pattern not in gitignore_content:
                    ...
                    repair_action['details'] = f'{file_pattern}は既に.gitignoreに含まれています'
        ...
        try:
            ...
            for pattern in temp_patterns:
                try:
                    ...
                    files_to_delete = [f for f in result.stdout.strip().split('\n') 
            ...
            repair_action['success'] = True
            repair_action['details'] = f'{len(cleaned_files)}個のファイル/ディレクトリを削除'
            
            space_freed_mb = repair_action['space_freed'] / (1024 * 1024)
            print(f"✅ ディスク容量清掃完了: {len(cleaned_files)}個削除, {space_freed_mb:.1f}MB解放")
    ...
    def auto_repair_detected_issues(self, security_alert: Dict) -> Dict:
        ...
        print(f"🔧 自動修復完了: {repair_summary['successful_repairs']}件成功, {repair_summary['failed_repairs']}件失敗")
...
"""
    ...
    def test_auto_repair_system(self) -> bool:
        ...
        print(f"{'✅' if success else '❌'} 自動修復システムテスト: {repair_result['successful_repairs']}件成功")
