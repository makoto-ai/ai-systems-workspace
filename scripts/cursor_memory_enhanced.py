...
class CursorEnhancedMemory(CursorMemorySystem):
    ...
    def record_enhanced_conversation(
        self,
        user_message: str,
        ai_response: str,
        context_before: str = "",
        ...
        conversation_id = hashlib.sha256(
            f"{timestamp.isoformat()}{user_message[:50]}".encode()
        ).hexdigest()[:8]
        ...
        detailed_file = (
            self.detailed_conversations
            / f"{conversation_id}_{timestamp.strftime('%Y%m%d_%H%M%S')}.json"
        )
    ...
    def _analyze_importance(self, user_message: str, ai_response: str) -> str:
        ...
        # 重要度チェック
        critical_count = sum(
            1 for keyword in self.importance_keywords["critical"] if keyword in text
        )
        high_count = sum(
            1 for keyword in self.importance_keywords["high"] if keyword in text
        )
        medium_count = sum(
            1 for keyword in self.importance_keywords["medium"] if keyword in text
        )
        contextual_count = sum(
            1 for keyword in self.importance_keywords["contextual"] if keyword in text
        )
    ...
    def _analyze_context(
        ...
        return {
            "continuation_detected": any(
                word in user_message.lower() for word in ["続き", "前回", "引き継ぎ"]
            ),
            "question_type": self._classify_question_type(user_message),
            "project_reference": self._extract_project_references(
                user_message, ai_response
            ),
            "technical_content": self._extract_technical_content(
                user_message, ai_response
            ),
            ...
            "context_quality": {
                ...
                "context_richness": len(context_before.split())
                + len(context_after.split()),
            },
        }
    ...
    def _analyze_emotional_context(
        ...
        return {
            ...
            "uncertainty_detected": any(
                ...
                for word in ["わからない", "不明", "？", "確認"]
            ),
            "satisfaction_indicators": any(
                word in user_message.lower()
                for word in ["ありがとう", "助かる", "完璧"]
            ),
            "frustration_indicators": any(
                word in user_message.lower()
                for word in ["勘弁", "困る", "怖い", "心配"]
            ),
    ...
    def _extract_entities(
        ...
        entities = {
            ...
            "system_names": re.findall(
                r"([A-Z][a-z]+(?:[A-Z][a-z]+)*(?:システム|System))", text
            ),
            "commands": re.findall(r"`([^`]+)`", text),
            "technologies": re.findall(
                r"(Python|JavaScript|SQL|Git|Docker|API|REST|JSON)", text
            ),
            "project_names": re.findall(
                r"(voice-roleplay|paper-research|monitoring)", text
            ),
        }
    ...
    def generate_auto_summary(self, timeframe_hours: int = 24) -> str:
        ...
        critical_conversations = [
            c for c in conversations if c["importance_level"] == "critical"
        ]
        high_conversations = [
            c for c in conversations if c["importance_level"] == "high"
        ]
...
"""
        ...
        summary_file = (
            self.auto_summaries
            / f"summary_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
        )
    ...
    def _extract_project_references(
        self, user_message: str, ai_response: str
    ) -> List[str]:
        """プロジェクト参照抽出"""
        text = f"{user_message} {ai_response}"
    ...
    def _extract_technical_content(
        self, user_message: str, ai_response: str
    ) -> Dict[str, Any]:
        """技術的内容抽出"""
        text = f"{user_message} {ai_response}"
        ...
        return {
            ...
            "file_operations": bool(
                any(word in text.lower() for word in ["削除", "作成", "変更", "移動"])
            ),
            "system_operations": bool(
                any(word in text.lower() for word in ["実行", "起動", "停止", "再起動"])
            ),
            "data_operations": bool(
                any(
                    word in text.lower()
                    for word in ["保存", "読み込み", "バックアップ", "復元"]
                )
            ),
        }
    ...
    def _detect_tone(self, message: str) -> str:
        ...
        if any(
            ...
            for word in ["ありがとう", "感謝", "助かる", "素晴らしい"]
        ):
            return "positive"
        elif any(
            word in message_lower for word in ["困る", "怖い", "心配", "勘弁", "不安"]
        ):
            return "negative"
    ...
    def _assess_confidence_level(self, user_message: str, ai_response: str) -> str:
        ...
        if any(word in ai_lower for word in ["確実", "間違いなく", "完璧", "100%"]):
            ...
            word in ai_lower for word in ["おそらく", "たぶん", "可能性", "思います"]
        ):
            return "medium"
    ...
    def _update_conversation_patterns(
        self, conversation_record: Dict[str, Any]
    ) -> None:
        """会話パターン学習"""
    ...
    def record_workflow_step(self, step_name: str, step_details: Dict[str, Any]) -> str:
        ...
        workflow_id = hashlib.sha256(
            f"{timestamp.isoformat()}{step_name}".encode()
        ).hexdigest()[:8]
        ...
        if any(
            ...
            for keyword in ["論文検索", "事実確認", "原稿作成", "youtube"]
        ):
            workflow_record["importance"] = "critical"

            # 論文検索→原稿作成フローの確認
            if "原稿作成" in step_name and not self._check_paper_search_prerequisite():
                workflow_record["warning"] = (
                    "論文検索システム未実行 - ハルシネーションリスク"
                )

        # 保存
        workflow_file = (
            self.workflow_memory
            / f"workflow_{workflow_id}_{timestamp.strftime('%Y%m%d_%H%M%S')}.json"
        )
    ...
    def _classify_workflow_type(
        self, step_name: str, step_details: Dict[str, Any]
    ) -> str:
        """ワークフロータイプ分類"""
        ...
        if any(keyword in step_lower for keyword in ["論文検索", "research", "paper"]):
            ...
            keyword in step_lower for keyword in ["原稿作成", "youtube", "script"]
        ):
            ...
            keyword in step_lower for keyword in ["システム構築", "system", "build"]
        ):
        ...
        for workflow_file in self.workflow_memory.glob("workflow_*.json"):
            try:
                with open(workflow_file, "r", encoding="utf-8") as f:
                    ...
                    workflow_time = datetime.datetime.fromisoformat(
                        workflow_data["timestamp"]
                    )

                    if (
                        workflow_time > cutoff_time
                        and workflow_data.get("workflow_type") == "research_workflow"
                    ):
                        return True
            except:
                continue
    ...
    def get_workflow_context(
        self, workflow_type: str = None, hours: int = 24
    ) -> Dict[str, Any]:
        """ワークフロー文脈取得"""
            ...
            try:
                with open(workflow_file, "r", encoding="utf-8") as f:
                    ...
                    if workflow_time > cutoff_time:
                        if (
                            workflow_type is None
                            or workflow_data.get("workflow_type") == workflow_type
                        ):
        ...
        return {
            ...
            "workflow_types": list(
                set(w.get("workflow_type", "unknown") for w in workflows)
            ),
            "critical_workflows": [
                w for w in workflows if w.get("importance") == "critical"
            ],
            "warnings": [w for w in workflows if "warning" in w],
            "recent_workflows": sorted(
                workflows, key=lambda x: x["timestamp"], reverse=True
            )[:5],
        }
