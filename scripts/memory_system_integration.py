...
class IntegratedMemorySystem:
    ...
    def record_conversation_with_consistency(self, user_message: str, ai_response: str, 
        ...
        # 2. 評価・発言の抽出と一貫性チェック
        consistency_results = self._process_consistency_checks(user_message, ai_response)
        
        # 3. 結果統合
        result = {
            "enhanced_conversation_id": enhanced_id,
            "consistency_checks": consistency_results,
            "safe_to_proceed": all(check.get("can_proceed", True) for check in consistency_results),
            "warnings": [warning for check in consistency_results for warning in check.get("warnings", [])]
        }
    ...
    def _process_consistency_checks(self, user_message: str, ai_response: str) -> list:
        ...
        for phrase in evaluation_phrases:
            ...
            check_result = self.consistency_engine.check_before_evaluation(context, phrase)
        ...
        if not evaluation_phrases:
            statement_id = self.consistency_engine.record_statement(ai_response, "general")
            results.append({"statement_id": statement_id, "can_proceed": True, "warnings": []})
        ...
        for pattern in evaluation_indicators:
            ...
            if matches:
                ...
                for sentence in sentences:
                    if any(match in sentence for match in matches if isinstance(match, str)):
                        phrases.append(sentence.strip())
                    elif any(any(m in sentence for m in match) for match in matches if isinstance(match, tuple)):
                        phrases.append(sentence.strip())
    ...
    def generate_memory_health_report(self) -> dict:
        ...
        # 既存記憶システムの状況
        enhanced_status = {
            "workflow_memory_files": len(list(self.enhanced_memory.workflow_memory.glob("*.json"))),
            "detailed_conversations": len(list(self.enhanced_memory.detailed_conversations.glob("*.json"))),
            "auto_summaries": len(list(self.enhanced_memory.auto_summaries.glob("*.md")))
        }
