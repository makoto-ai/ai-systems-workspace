...
class DataIntegrityChecker:
    ...
    def check_backup_integrity(self):
        ...
        backup_locations.extend([str(f) for f in bak_files if "node_modules" not in str(f) and ".venv" not in str(f)])
        
        # .backupファイル  
        backup_files = list(base_dir.glob("**/*.backup"))
        backup_locations.extend([str(f) for f in backup_files if "node_modules" not in str(f) and ".venv" not in str(f)])
        ...
        check_result = {
            ...
            "details": {
                ...
                "status_message": f"Found {len(backup_locations)} backup files" if len(backup_locations) > 0 else "No backup files found"
            }
        }
                    ...
                    check_result.update({
                        ...
                        "details": {
                            "usage_limits_json": {
                                "exists": True,
                                "user_count": len(data),
                                "total_entries": sum(len(user_data) for user_data in data.values()) if isinstance(data, dict) else 0,
                                "file_size_kb": round(usage_limits_file.stat().st_size / 1024, 2)
                            }
                        }
                    })
