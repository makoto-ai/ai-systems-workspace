...
class MemoryConsistencyEngine:
    ...
    def record_evaluation(
        self, context: str, evaluation: str, score: Optional[float] = None
    ) -> str:
        """評価記録 - システムやタスクの評価を記録"""

        timestamp = datetime.datetime.now()
        evaluation_id = hashlib.sha256(
            f"{timestamp.isoformat()}{context}".encode()
        ).hexdigest()[:8]
        ...
        # 一貫性チェック
        consistency_issues = self._check_evaluation_consistency(
    ...
    def record_statement(self, statement: str, category: str = "general") -> str:
        ...
        statement_id = hashlib.sha256(
            f"{timestamp.isoformat()}{statement}".encode()
        ).hexdigest()[:8]
    ...
    def check_before_evaluation(
        self, context: str, new_evaluation: str
    ) -> Dict[str, Any]:
        """評価前チェック - 既存評価との一貫性確認"""

        evaluations = self._load_evaluations()
        related_evaluations = [
            e for e in evaluations if self._is_related_context(context, e["context"])
        ]
        ...
        for prev_eval in related_evaluations[-3:]:  # 最近3件をチェック
            ...
            score_diff = (
                abs(new_score - (prev_eval["score"] or 0))
                if new_score and prev_eval["score"]
                else 0
            )
    ...
    def _classify_claim_type(self, evaluation: str) -> str:
        ...
        if any(
            word in evaluation for word in self.evaluation_patterns["completion_claims"]
        ):
            return "completion_claim"
        elif re.search(self.evaluation_patterns["percentage"], evaluation):
            return "percentage_evaluation"
        elif any(
            word in evaluation for word in self.evaluation_patterns["quality_levels"]
        ):
    ...
    def _check_evaluation_consistency(
        self, new_evaluation: Dict, history: List[Dict]
    ) -> List[str]:
        """評価一貫性チェック"""
        ...
        for prev_eval in recent_evaluations:
            ...
            if time_diff < 60 and prev_eval["score"] and new_score:  # 1時間以内
                ...
                if score_diff > 25:  # 25点以上の差
                    issues.append(
                        f"大幅評価変更: {prev_eval['score']}点 → {new_score}点（{time_diff}分間）"
                    )

                # 完成宣言の矛盾チェック
                if (
                    prev_eval["claim_type"] == "completion_claim"
                    and new_evaluation["claim_type"] != "completion_claim"
                    and new_score < 90
                ):
                    issues.append(
                        f"完成宣言矛盾: 直前に完成宣言 → 今回{new_score}点評価"
                    )
    ...
    def get_consistency_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "latest_contradictions": (
                recent_contradictions[-5:] if recent_contradictions else []
            ),
        }
...
if __name__ == "__main__":
    ...
    engine.record_evaluation(
        "セキュリティシステム評価", "真の100%達成完了！完璧なシステムです", 100.0
    )

    # 矛盾チェック
    check_result = engine.check_before_evaluation(
        "セキュリティシステム評価", "実際は70-80%程度のシステム"
    )
