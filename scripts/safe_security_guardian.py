...
class SafeSecurityGuardian:
    ...
    def load_safe_config(self):
        ...
        default_config = {
            ...
            "never_auto_modify": ["database/", "backups/", ".git/", "config/"],
        }
    ...
    def scan_with_safety_checks(self) -> Dict[str, Any]:
        ...
        files_to_scan = list(self.project_root.glob("**/*.py"))[
            : self.config["max_files_per_scan"]
        ]
        ...
        print(
            f"✅ 安全スキャン完了: {scan_results['files_scanned']}ファイル、{len(scan_results['potential_issues'])}件の潜在的問題"
        )
    ...
    def run_safe_monitoring(self):
        ...
        print(
            f"✋ 手動承認: {'必須' if self.config['require_manual_approval'] else '不要'}"
        )
        ...
        while True:
            try:
                ...
                print(
                    f"📊 リソース: CPU {usage['cpu_percent']:.1f}%, RAM {usage['memory_mb']:.1f}MB"
                )
