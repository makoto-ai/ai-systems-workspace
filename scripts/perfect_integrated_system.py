...
class PerfectIntegratedSystem:
    ...
    async def run_paper_research_with_security(
        self, query: str, max_results: int = 10, save_obsidian: bool = True
    ) -> Dict[str, Any]:
        """セキュリティ統合論文検索実行"""
        ...
        try:
            ...
            result = subprocess.run(
                cmd, capture_output=True, text=True, cwd=paper_system_path
            )
    ...
    async def run_youtube_script_with_security(
        self, topic: str, title: Optional[str] = None
    ) -> Dict[str, Any]:
        """セキュリティ統合YouTube原稿作成"""
        ...
        try:
            ...
            if result.get("success"):
                ...
                return {"status": "ERROR", "error": result.get("error", "不明なエラー")}
    ...
    async def run_complete_pipeline(
        self, topic: str, youtube_title: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        pipeline_result = {"status": "RUNNING", "steps": [], "final_outputs": {}}

        # Step 1: 論文検索
        print("\n🔍 Step 1: 論文検索実行...")
        paper_result = await self.run_paper_research_with_security(
            query=topic, max_results=10, save_obsidian=True
        )
        ...
        youtube_result = await self.run_youtube_script_with_security(
            topic=topic, title=youtube_title
        )
