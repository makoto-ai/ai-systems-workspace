...
class SecurityFullAutomation:
    ...
    def start_full_automation(self):
        ...
        file_monitor_thread = threading.Thread(
            target=self._file_change_monitor, daemon=True
        )
        ...
        full_scan_thread = threading.Thread(
            target=self._periodic_full_scan, daemon=True
        )
        full_scan_thread.start()
        self.monitoring_threads.append(full_scan_thread)

        # 3. 学習エンジンスレッド
        learning_thread = threading.Thread(target=self._periodic_learning, daemon=True)
        ...
        while self.is_running and retry_count < max_retries:
            try:
                ...
                for changes in watch(str(self.project_root)):
                    ...
                    if changed_files:
                        print(
                            f"🔍 セキュリティスキャン対象: {len(changed_files)}ファイル"
                        )

                        # 30秒待機してから自動スキャン
                        print(
                            f"⏳ {self.intervals['file_scan']}秒待機後にスキャン実行..."
                        )
    ...
    def _should_scan_file(self, file_path: str) -> bool:
        ...
        exclude_dirs = [
            ...
            "__pycache__",
            ".pytest_cache",
            "frontend/voice-roleplay-frontend/node_modules",
        ]
        ...
        try:
            ...
            if free_space_gb < 1.0:  # 1GB未満
                ...
                health_status["issues"].append(
                    f"ディスク容量不足: {free_space_gb:.1f}GB"
                )
