...
- out/prev_shadow_grid.json (baseline snapshot prepared by CI)
- out/kpi_history.jsonl (fallback for previous pass rate when snapshot missing)
"""
...
def load_previous_metrics() -> Optional[Dict[str, float]]:
    # Prefer explicit previous shadow grid snapshot written by CI
        ...
        try:
            ...
            if lines:
                ...
                return {
                    ...
                    "new_fail": 100.0,  # unknown -> neutral worst
                }
        except Exception:
            pass
    return None
...
def decide(current: Dict[str, float], previous: Optional[Dict[str, float]]) -> Dict[str, Any]:
    ...
    if prev_new is not None and prev_new < 100.0:
    ...
    return {
        ...
        "current": current,
        "previous": previous,
        "reason": ", ".join(improvements) if accepted else ("; ".join(regressions) or "No measurable improvement"),
    }
