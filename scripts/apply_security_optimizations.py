...
class SecurityOptimizationApplicator:
    ...
    def __init__(self, project_root: str = "."):
        ...
        self.optimization_configs = {
            ...
            "memory_integration": self.scripts_dir / "memory_security_integration_config.json",
            "github_actions": self.scripts_dir / "github_actions_optimization_config.json"
        }
    ...
    def apply_threat_detection_optimizations(self) -> Dict[str, Any]:
        ...
        if re.search(old_exclusions_pattern, content, re.DOTALL):
            content = re.sub(old_exclusions_pattern, new_exclusions, content, flags=re.DOTALL)
        ...
        if "calculate_threat_level" in content:
            ...
            medium_threshold = adjusted_thresholds["file_permissions"]["medium_threshold"]
            
            # 閾値パターンの更新
            content = re.sub(
                r'high_count\s*>\s*\d+',
                f'high_count > {high_threshold}',
                content
            )
            content = re.sub(
                r'medium_count\s*>\s*\d+',
                f'medium_count > {medium_threshold}',
                content
            )
        ...
        if "def detect_sensitive_data_exposure" in content:
            ...
            if function_start != -1:
                content = content[:function_start+1] + safe_pattern_code + content[function_start+1:]
...
'''
    ...
    def verify_optimizations(self) -> Dict[str, Any]:
        ...
        verification_results["total_optimizations"] = len(verification_results["config_files"])
        
        print(f"✅ 最適化確認完了: {verification_results['total_optimizations']}個の最適化適用")
