...
class Config:
    ...
    GOOGLE_GENERATIVE_AI_API_KEY: Optional[str] = os.getenv(
    ...
    ENABLE_VOICE_CLONING: bool = (
        os.getenv("ENABLE_VOICE_CLONING", "false").lower() == "true"
    )
    ENABLE_SPEAKER_DIARIZATION: bool = (
        os.getenv("ENABLE_SPEAKER_DIARIZATION", "false").lower() == "true"
    )
    ENABLE_ADVANCED_VOICE_ANALYSIS: bool = (
        os.getenv("ENABLE_ADVANCED_VOICE_ANALYSIS", "true").lower() == "true"
    )
    ...
    def check_warnings(cls):
        ...
        if cls.ENABLE_SPEAKER_DIARIZATION and not cls.HUGGINGFACE_HUB_TOKEN:
            warnings.append(
                "HuggingFace token not set - speaker diarization will be disabled"
            )
