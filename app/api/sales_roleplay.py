...
try:
    from core.sales_speakers import (
        SALES_SPEAKERS, SALES_SCENARIOS,
        get_sales_speaker_list, get_speaker_by_scenario, get_all_scenarios
    )
...
class SalesTextToSpeechRequest(BaseModel):
    ...
    scenario: str = Field(..., description="営業シナリオ（new_business, existing_customer, etc.）")
    gender: Optional[str] = Field("random", description="スピーカーの性別（male, female, random）")
    emotion_level: Optional[str] = Field("normal", description="感情レベル（normal, intense, gentle）")
    ...
    def scenario_must_be_valid(cls, v: str) -> str:
        if v not in SALES_SCENARIOS:
            ...
            raise ValueError(f'無効なシナリオです。次の中から選択: {", ".join(valid_scenarios)}')
        return v
...
async def get_sales_speakers() -> Dict[str, Any]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="スピーカー一覧の取得に失敗しました")
...
async def get_recommended_speaker(
    ...
    try:
        if scenario_id not in SALES_SCENARIOS:
            raise HTTPException(status_code=404, detail="指定されたシナリオが見つかりません")
        ...
        raise HTTPException(status_code=500, detail="推奨スピーカーの取得に失敗しました")

@router.post("/text-to-speech", summary="営業特化音声合成")
async def sales_text_to_speech(request: Request, req: SalesTextToSpeechRequest) -> StreamingResponse:
    """営業シナリオに基づいた音声合成"""
    try:
        # シナリオに基づいてスピーカーを選択
        speaker = get_speaker_by_scenario(req.scenario, req.gender)
        if not speaker:
            raise HTTPException(status_code=404, detail="指定されたシナリオに対応するスピーカーが見つかりません")
        ...
        logger.info(f"営業音声合成成功: scenario={req.scenario}, speaker={speaker['name']}, text_length={len(req.text)}")
        
        # HTTPヘッダーは日本語をサポートしないため、Base64エンコード
        import base64
        speaker_name_b64 = base64.b64encode(speaker["name"].encode('utf-8')).decode('ascii')
        speaker_character_b64 = base64.b64encode(speaker["character"].encode('utf-8')).decode('ascii')
        ...
        raise HTTPException(status_code=500, detail="音声合成処理中にエラーが発生しました")
