...
async def create_prompt_template(template: PromptTemplate) -> Dict[str, Any]:
    ...
    try:
        ...
        # ファイルに保存
        template_file = PROMPT_TEMPLATES_DIR / f"{template.name.replace(' ', '_').lower()}.json"
        ...
        raise HTTPException(status_code=500, detail=f"テンプレート作成に失敗しました: {e}")
...
async def get_prompt_template(template_id: str) -> Dict[str, Any]:
    ...
    if template_file.exists():
        try:
            ...
            raise HTTPException(status_code=500, detail=f"テンプレート読み込みエラー: {e}")
...
async def optimize_prompt(request: PromptOptimizationRequest) -> Dict[str, Any]:
    ...
    try:
        # 最適化戦略を選択
        strategies = optimization_strategies.get(request.target_scenario, optimization_strategies["sales"])
...
"""
        ...
        return {
            ...
            "improvements": [strategies[goal] for goal in request.quality_goals if goal in strategies]
        }
...
async def test_prompt_quality(
    ...
    try:
        ...
        quality_score = _evaluate_response_quality(response_text, customer_type, industry)
        ...
        return {
            ...
            "quality_metrics": {
                ...
                "politeness_detected": any(word in response_text for word in ["ございます", "いただき", "申し上げ"]),
                "question_included": "？" in response_text or "でしょうか" in response_text,
                "industry_relevant": industry.lower() in response_text.lower() if industry != "general" else True
            }
        }
...
def _evaluate_response_quality(response: str, customer_type: str, industry: str) -> float:
    """応答品質を評価（0-1のスコア）"""
    score = 0.0
    ...
    # 質問・確認の含有 (0.2)
    if "？" in response or any(word in response for word in ["でしょうか", "いかがでしょう", "お聞かせ"]):
        score += 0.2
...
async def get_prompt_analytics() -> Dict[str, Any]:
    ...
    return {
        "total_templates": len(DEFAULT_TEMPLATES) + len(list(PROMPT_TEMPLATES_DIR.glob("*.json"))),
