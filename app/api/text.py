...
try:
    from services.swallow_text_service import get_swallow_service, SwallowTextService, TextAnalysisResult
except ImportError:
    from app.services.swallow_text_service import get_swallow_service, SwallowTextService, TextAnalysisResult
...
async def upload_and_analyze_file(
    ...
    try:
        ...
        if text_length > MAX_TEXT_LENGTH:
            logger.warning(f"Text length ({text_length}) exceeds limit ({MAX_TEXT_LENGTH}), truncating...")
        ...
        if analysis_type == "comprehensive" and QUALITY_PROCESSING_ENABLED:
            ...
            return {
                ...
                "file_info": {
                    ...
                    "pages_estimated": processed_result.get("word_count", 0) // 250,  # 250語/ページ想定
...
async def batch_analyze_files(
    ...
    try:
        ...
        logger.info(f"Processing batch: {len(files)} files, total {total_size_mb:.1f}MB")
        ...
        return {
            ...
            "batch_info": {
                ...
                "successful_files": len([r for r in batch_results if r.get("processing_status") == "success"]),
                "failed_files": len([r for r in batch_results if r.get("processing_status") == "error"]),
                "processing_mode": "high_quality" if integrated_analysis else "standard"
            },
...
async def analyze_google_docs(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Google Docs analysis failed: {str(e)}")
...
async def analyze_google_sheets(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Google Sheets analysis failed: {str(e)}")
...
async def get_supported_file_formats() -> Dict[str, Any]:
    ...
    return {
        ...
        "source_code": {
            "languages": [".py", ".js", ".ts", ".java", ".cpp", ".c", ".h", ".cs", ".php", ".rb", ".go", ".rs"],
            "scripts": [".sh", ".sql", ".css"],
