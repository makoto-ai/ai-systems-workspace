...
async def initialize_session(
    ...
    try:
        # プライバシーモード検証
        valid_modes = {"strict": PrivacyMode.STRICT, "standard": PrivacyMode.STANDARD, "research": PrivacyMode.RESEARCH}
        if privacy_mode not in valid_modes:
            raise HTTPException(status_code=400, detail=f"Invalid privacy mode: {privacy_mode}")
...
async def get_contextual_suggestions(request: ContextualSuggestionRequest) -> Dict[str, Any]:
    """
    文脈に基づいた提案を取得
