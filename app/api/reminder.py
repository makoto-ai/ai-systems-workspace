...
try:
    ...
    from services.reminder_scheduler_service import (
        get_reminder_scheduler_service,
...
async def update_reminder_settings(
    ...
    try:
        ...
        if result.get("success"):
            ...
            raise HTTPException(
                status_code=500, detail=result.get("error", "設定更新に失敗しました")
            )
...
async def record_roleplay_session(
    ...
    try:
        ...
        if result.get("success"):
            ...
            raise HTTPException(
                status_code=500,
                detail=result.get("error", "セッション記録に失敗しました"),
            )
...
async def get_user_confrontation_report(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"突きつけレポート取得エラー: {str(e)}"
        )
...
async def get_all_users_activity_report(
    ...
    try:
        ...
        if not report.get("success"):
            raise HTTPException(
                status_code=500,
                detail=report.get("error", "レポート生成に失敗しました"),
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"全ユーザーレポート取得エラー: {str(e)}"
        )
...
async def send_shame_notification(
    ...
    usage_service: UsageLimitService = Depends(get_usage_limit_service),
    notification_service: MultiChannelNotificationService = Depends(
        get_notification_service
    ),
) -> Dict[str, Any]:
    """
...
"""
        ...
        send_results = await notification_service.send_multi_channel(
            message=notification_message, channels=channels
        )

        logger.info(
            f"Shame notification sent to user {user_id} via channels: {channels}"
        )
...
async def start_reminder_scheduler(
    ...
    try:
        ...
        if result.get("success"):
            ...
            raise HTTPException(
                status_code=500,
                detail=result.get("message", "スケジューラー開始に失敗しました"),
            )
...
async def manual_send_reminders(
    ...
    try:
        ...
        if result.get("success"):
            ...
            raise HTTPException(
                status_code=500,
                detail=result.get("error", "リマインダー送信に失敗しました"),
            )
...
async def get_reminder_dashboard(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"ダッシュボード取得エラー: {str(e)}"
        )
...
async def startup_reminder_scheduler():
    ...
    try:
        ...
        if result.get("success"):
            ...
            logger.warning(
                f"Failed to auto-start reminder scheduler: {result.get('message')}"
            )
...
async def test_multiple_channels(
    ...
    try:
        ...
        # 複数チャンネルに送信
        result = await notification_service.send_multi_channel(
            test_message, test_request.channels
        )
        ...
        return {
            ...
            "summary": {
                ...
                "failed": result.get("total_channels", 0)
                - result.get("successful_channels", 0),
            },
        }
        ...
        raise HTTPException(
            status_code=500, detail=f"マルチチャンネルテストエラー: {str(e)}"
        )
...
async def send_custom_notification(
    ...
    try:
        ...
        if result.get("success"):
            logger.info(
                f"Custom notification sent to {result.get('successful_channels')} channels for user {user_id}"
            )
            return {
                "success": True,
