...
async def upload_and_analyze_video(
    ...
    try:
        ...
        # Check usage limits
        can_process_result = await usage_service.can_process_video(user_id, estimated_duration_minutes)
        if not can_process_result["can_process"]:
            raise HTTPException(
                status_code=429,
                detail=can_process_result["message"]
            )
        
        # Consume usage quota
        consumption_result = await usage_service.consume_video_processing(user_id, estimated_duration_minutes)
        if not consumption_result["success"]:
            raise HTTPException(
                status_code=429,
                detail=consumption_result["message"]
            )
        ...
        return {
            ...
            "usage_info": {
                ...
                "roleplay_sessions_consumed": consumption_result["roleplay_sessions_consumed"],
                "remaining_video_minutes": consumption_result["remaining_video_minutes"],
                "remaining_roleplay_sessions": consumption_result["remaining_roleplay_sessions"]
            },
...
async def admin_get_all_users_usage(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get all usage: {str(e)}") 