...
try:
    from services.language_analysis_service import (
        language_analysis_service,
        PolitenessLevel,
        LanguageIssueType,
    )
    from services.friendliness_analyzer import friendliness_analyzer, FriendlinessLevel
except ImportError:
    from app.services.language_analysis_service import (
        language_analysis_service,
        PolitenessLevel,
        LanguageIssueType,
    )
    from app.services.friendliness_analyzer import (
        friendliness_analyzer,
        FriendlinessLevel,
    )
...
async def analyze_language_quality(
    ...
    try:
        ...
        result = await language_analysis_service.analyze_language_quality(
            text=text, context=context, customer_level=customer_level
        )
        ...
        return {
            ...
            "summary": {
                ...
                "business_appropriateness": result["business_appropriateness"][
        ...
        raise HTTPException(
            status_code=500, detail=f"言語分析でエラーが発生しました: {str(e)}"
        )


@router.post("/language/check-politeness")
async def check_politeness_level(text: str = Form(...)):
    """
    丁寧度レベルのクイックチェック
...
async def get_language_fixes(
    text: str = Form(...), priority: str = Form(default="high")
):
    """
    言葉遣いの修正提案を取得
    ...
    try:
        ...
        return {
            ...
            "improvement_count": (
                len(selected_improvements)
                if isinstance(selected_improvements, list)
                else sum(len(v) for v in selected_improvements.values())
            ),
        }
...
async def check_keigo_usage(text: str = Form(...)):
    """
    敬語使用状況のチェック
...
async def analyze_communication_tone(
    text: str = Form(...), target_tone: str = Form(default="professional_friendly")
):
    """
    コミュニケーショントーンの分析
...
def _check_industry_specific_language(text: str, industry: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "industry_fit_score": (preferred_count + technical_count)
        / max(len(patterns["preferred"]) + len(patterns["technical"]), 1),
    }
...
def _generate_business_recommendations(
    ...
    if business_analysis["appropriateness_score"] < 0.7:
        recommendations.append(
            f"{customer_level}レベルの顧客により適した表現を使用してください"
        )
    ...
    if industry != "general":
        recommendations.append(
            f"{industry}業界特有の表現を取り入れることを検討してください"
        )
...
async def analyze_friendliness(
    ...
    try:
        ...
        result = friendliness_analyzer.analyze_friendliness(
            text=text, context=context, customer_relationship=customer_relationship
        )
        ...
        return {
            ...
            "improvement_tips": friendliness_analyzer.get_friendliness_tips(
                result.level
            ),
        }
        ...
        raise HTTPException(
            status_code=500, detail=f"親しみやすさ分析でエラーが発生しました: {str(e)}"
        )
...
async def get_warmth_suggestions(
    text: str = Form(...), current_issues: List[str] = Form(default=[])
):
    """
    温かみ向上の具体的提案
...
def _assess_distance_balance(
    friendliness_result, language_result, target_relationship: str
) -> Dict[str, Any]:
    """距離感バランスの評価"""
    ...
    config = target_configs.get(
        target_relationship, target_configs["friendly_professional"]
    )

    # バランススコア計算
    politeness_match = (
        1.0
        if language_result["politeness_analysis"]["level"]
        == config["target_politeness"]
        else 0.5
    )
    friendliness_match = (
        1.0 if friendliness_result.level.value == config["target_friendliness"] else 0.5
    )
    ...
    if balance_score < 0.6:
        ...
        recommendations.append(
            f"{target_relationship}に適したバランスに調整してください"
        )
...
def _generate_warmth_improvement_suggestions(
    text: str, friendliness_result, current_issues: List[str]
) -> Dict[str, List[str]]:
    """温かみ向上提案の生成"""
...
def _generate_before_after_examples(
    text: str, friendliness_result
) -> List[Dict[str, str]]:
    """改善前後の例文生成"""
...
def _calculate_tone_adjustment(
    current_tone: Dict[str, Any], target_tone: str
) -> Dict[str, float]:
    """トーン調整の計算"""
    target_mapping = {
    ...
    target_scores = target_mapping.get(
        target_tone, target_mapping["professional_friendly"]
    )
