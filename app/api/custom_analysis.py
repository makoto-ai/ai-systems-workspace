...
class CustomAnalysisService:
    ...
    async def analyze_with_custom_perspective(
        ...
        for criterion_name, criterion_config in criteria.items():
            score = self._evaluate_criterion(
                conversation_text, criterion_config, industry
            )
        ...
        # 総合スコア計算
        weighted_score = sum(
            result["score"] * result["weight"] for result in analysis_results.values()
        )

        # 個別フィードバック生成
        feedback = self._generate_personalized_feedback(
            analysis_results, weighted_score, skill_level
        )

        # 次回の推奨事項
        recommendations = self._generate_next_session_recommendations(
            analysis_results, focus_areas
        )
    ...
    def _evaluate_criterion(
        self, text: str, criterion_config: Dict[str, Any], industry: str
    ) -> float:
        """評価基準に基づくスコア計算"""
        ...
        # 業界特化の調整
        industry_bonus = self._calculate_industry_bonus(
            text, industry, criterion_config
        )
    ...
    def _calculate_industry_bonus(
        self, text: str, industry: str, criterion_config: Dict[str, Any]
    ) -> float:
        """業界特化ボーナス"""
        ...
        if industry_keywords_key in criterion_config:
            ...
            industry_mentions = sum(
                1 for keyword in industry_keywords if keyword in text
            )
    ...
    def _generate_personalized_feedback(
        self, analysis_results: Dict[str, Any], overall_score: float, skill_level: str
    ) -> Dict[str, Any]:
        """個人化されたフィードバック生成"""
        ...
        if overall_score >= 0.8:
            ...
            main_feedback = template.format(
                basic_skill="基本的な営業スキル", detailed_guidance="段階的な練習"
            )
    ...
    def _get_industry_insights(self, industry: str, text: str) -> Dict[str, Any]:
        ...
        if industry in industry_config.get("industry_mappings", {}):
            ...
            # 業界特有の懸念事項の言及チェック
            concerns_mentioned = [
                concern for concern in industry_info["key_concerns"] if concern in text
            ]

            # 成功要因の言及チェック
            success_factors_mentioned = [
                factor for factor in industry_info["success_factors"] if factor in text
            ]
            ...
            return {
                ...
                / (
                    len(industry_info["key_concerns"])
                    + len(industry_info["success_factors"])
                ),
                "recommendations": self._get_industry_recommendations(
    ...
    def _get_industry_recommendations(
        self, industry: str, industry_info: Dict[str, Any]
    ) -> List[str]:
        """業界特化の推奨事項"""
        recommendations = []
    ...
    def _generate_next_session_recommendations(
        self, analysis_results: Dict[str, Any], focus_areas: Optional[List[str]] = None
    ) -> Dict[str, Any]:
        """次回セッションの推奨事項"""

        # 最も低いスコアの分野を特定
        lowest_score_area = min(
            analysis_results.keys(), key=lambda x: analysis_results[x]["score"]
        )
    ...
    def _recommend_difficulty_level(self, analysis_results: Dict[str, Any]) -> str:
        """難易度レベルの推奨"""
        avg_score = sum(result["score"] for result in analysis_results.values()) / len(
            analysis_results
        )
    ...
    def _recommend_practice_exercises(self, focus_area: str) -> List[str]:
        ...
        exercises_map = {
            ...
            "specificity_score": ["数字を使った説明練習", "具体例を交えた事例紹介練習"],
            "closing_power": ["決断を促す表現の練習", "次のステップ提示の練習"],
            "industry_knowledge": ["業界トレンドの学習", "競合他社の分析練習"],
        }
...
async def update_custom_config(
    ...
    try:
        ...
        return {
            ...
            "config_name": config_data.get("custom_analysis_config", {}).get(
                "analyzer_name", "カスタム分析"
            ),
        }
...
async def bulk_analyze_conversations(
    ...
    try:
        ...
        for i, conversation in enumerate(conversations):
            ...
            results.append({"conversation_id": i + 1, "analysis": result})

        # 全体の統計情報
        overall_stats = {
            "total_conversations": len(conversations),
            "average_score": sum(r["analysis"]["overall_score"] for r in results)
            / len(results),
