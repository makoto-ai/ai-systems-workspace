...
try:
    from services.conversation_service import get_conversation_service, ConversationServiceError
    from services.usage_limit_service import get_usage_limit_service
except ImportError:
    from app.services.conversation_service import get_conversation_service, ConversationServiceError
...
class VoiceConversationRequest(BaseModel):
    ...
    speaker_preferences: Optional[Dict[str, Any]] = Field(None, description="Speaker preferences")
...
async def handle_objection(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Objection handling failed: {str(e)}")
...
async def get_recent_sessions(
    ...
    try:
        ...
        recent_sessions = await conversation_service.history_service.get_recent_sessions(
            hours=hours,
            limit=limit
        )
...
async def create_session(
    ...
    try:
        ...
        # Get session context
        session_context = await conversation_service.history_service.get_session_context(session_id)
...
async def get_session(
    ...
    try:
        ...
        if include_history:
            conversation_history = await conversation_service.history_service.get_conversation_history(
                session_id=session_id,
                limit=history_limit,
                include_audio=False
            )
...
async def update_sales_stage(
    ...
    try:
        ...
        if not success:
            raise HTTPException(status_code=404, detail="Session not found or invalid stage")
        
        return {
            "success": True,
            "message": f"Sales stage updated to {new_stage}"
        }
        ...
        raise HTTPException(status_code=500, detail=f"Failed to update sales stage: {str(e)}")
...
async def update_bant_status(
    ...
    try:
        ...
        return {
            "success": True,
            "message": "BANT status updated successfully"
        }
        ...
        raise HTTPException(status_code=500, detail=f"Failed to update BANT status: {str(e)}") 
...
async def end_session_analysis(
    ...
    try:
        ...
        friendliness_analysis = {
            "overall_score": getattr(friendliness_analysis_result, 'overall_score', 0.0),
            "warmth_score": getattr(friendliness_analysis_result, 'warmth_score', 0.0),
            "empathy_score": getattr(friendliness_analysis_result, 'empathy_score', 0.0),
            "naturalness_score": getattr(friendliness_analysis_result, 'naturalness_score', 0.0),
            "level": getattr(friendliness_analysis_result, 'level', 'unknown'),
            "recommendations": getattr(friendliness_analysis_result, 'recommendations', [])
        }
...
async def _generate_comprehensive_report(
    ...
    # 営業スキル評価
    sales_skills = {
        "overall_score": sales_analysis.get("overall_assessment", {}).get("overall_score", 0.0),
        "rapport_building": sales_analysis.get("basic_analysis", {}).get("rapport_building", {}),
        "needs_discovery": sales_analysis.get("basic_analysis", {}).get("needs_discovery", {}),
        "value_proposition": sales_analysis.get("basic_analysis", {}).get("value_proposition", {}),
        "objection_handling": sales_analysis.get("basic_analysis", {}).get("objection_handling", {})
    }
    
    # 言語品質評価
    language_quality = {
        "overall_score": language_analysis.get("overall_score", 0.0),
        "politeness_level": language_analysis.get("politeness_analysis", {}).get("level", ""),
        "business_appropriateness": language_analysis.get("business_appropriateness", {}).get("appropriateness_score", 0.0),
...
def _calculate_overall_performance(sales_score: float, language_score: float, friendliness_score: float) -> Dict[str, Any]:
    """総合パフォーマンス評価"""
    
    # 重み付け平均 (営業スキル40%, 言語品質35%, 親しみやすさ25%)
    weighted_score = (sales_score * 0.4) + (language_score * 0.35) + (friendliness_score * 0.25)
...
def _identify_next_focus_areas(sales_skills: Dict, language_quality: Dict, friendliness_quality: Dict) -> List[str]:
    """次回のフォーカスエリア特定"""
...
def _create_skill_development_plan(sales_skills: Dict, language_quality: Dict, friendliness_quality: Dict) -> Dict[str, Any]:
    """スキル開発計画の作成"""
...
def _generate_specific_feedback(sales_analysis: Dict, language_analysis: Dict, friendliness_analysis: Dict) -> List[str]:
    """具体的なフィードバック生成"""
