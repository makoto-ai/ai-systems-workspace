...
class SpeechService:
    ...
    def __init__(self, model_size: str = "tiny", language: str = "ja", device: str = "auto", hf_token: Optional[str] = None):
        """
    ...
    def _load_diarization_model_on_demand(self) -> bool:
        ...
        if not self.hf_token:
            logger.warning("Cannot load diarization model: HuggingFace token not provided")
            return False
    ...
    def _format_result_fast(self, result: Dict[str, Any], language: str) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "confidence": sum(s.get("confidence", 0) for s in processed_segments) / max(len(processed_segments), 1),
    ...
    def _format_result(
        ...
        total_duration = processed_segments[-1]["end"] if processed_segments else 0.0
        avg_confidence = sum(seg.get("confidence", 0.0) for seg in processed_segments) / len(processed_segments) if processed_segments else 0.0
        
        return {
            "text": full_text.strip(),
            "language": language,
            "segments": processed_segments,
            "confidence": max(0.0, min(1.0, (avg_confidence + 10) / 10)),  # Convert log prob to 0-1 scale
            "duration": total_duration,
...
def get_speech_service(model_size: str = "base", language: str = "ja", hf_token: Optional[str] = None) -> SpeechService:
    """
