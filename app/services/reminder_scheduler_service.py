...
try:
    ...
    from services.notification_service import MultiChannelNotificationService, NotificationMessage
except ImportError:
    ...
    from app.services.notification_service import MultiChannelNotificationService, NotificationMessage
...
class ReminderSchedulerService:
    ...
    async def _process_reminders(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if now.weekday() == 0:  # 月曜日
                ...
                results["escalation_notifications"] = escalation_result.get("sent_count", 0)
    ...
    async def _process_escalation_notifications(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Found {len(escalation_users)} users for escalation notifications")
            ...
            for user in escalation_users:
                try:
                    ...
                    # ユーザーの詳細統計取得
                    user_stats = await self.usage_service.get_user_roleplay_stats(user_id)
                    confrontation_data = user_stats.get("confrontation_data", {})
                    
                    # エスカレーションメッセージ作成
                    escalation_message = self._create_escalation_message(user, confrontation_data)
                    
                    # エスカレーションメール送信
                    email_result = await self.email_service.send_reminder_email(
                        email_address=user_stats.get("reminder_settings", {}).get("email_address", ""),
                    ...
                    if email_result.get("success"):
                        ...
                        errors.append(f"Failed to send escalation to {user_id}: {email_result.get('error')}")
                        
                except Exception as e:
                    errors.append(f"Error sending escalation to {user.get('user_id', 'unknown')}: {str(e)}")
    ...
    def _create_escalation_message(self, user: Dict[str, Any], confrontation_data: Dict[str, Any]) -> str:
        """
        エスカレーション用のメッセージを作成
        """
...
"""
    ...
    async def _send_reminder_batch(self, users: List[Dict[str, Any]], reminder_type: str) -> Dict[str, Any]:
        """リマインダーメールのバッチ送信"""
        errors = []
        for user_data in users:
            try:
                # パーソナライズされたメッセージを生成
                personalized_message = self._generate_personalized_message(user_data, reminder_type)
                ...
                if success:
                    logger.info(f"Sent {reminder_type} reminder to {user_data['email']}")
                else:
                    logger.error(f"Failed to send {reminder_type} reminder to {user_data['email']}")
                
                # メール送信間隔
                await asyncio.sleep(1)
                
            except Exception as e:
                logger.error(f"Error sending reminder to {user_data.get('email', 'unknown')}: {e}")
                ...
                errors.append(f"Failed to send {reminder_type} reminder to {user_data.get('email', 'unknown')}: {str(e)}")
        
        # ログ保存
        self._save_logs()
        return {"success": True, "sent_count": len(users), "errors": errors}
    
    def _generate_personalized_message(self, user_data: Dict[str, Any], reminder_type: str) -> str:
        """パーソナライズされたメッセージを生成"""
        try:
            ...
            if total_sessions >= 20:
                performance_messages.append("20回以上のセッション、かなりの経験を積まれています。")
            elif total_sessions >= 10:
                performance_messages.append("10回以上のセッション、着実にスキルが向上していますね。")
            
            # 改善ポイントに基づくメッセージ
            improvement_points = user_data.get("improvement_points", [])
            if improvement_points:
                recent_point = improvement_points[-1]
                performance_messages.append(f"前回は「{recent_point}」の改善を目指していましたね。")
    ...
    async def manual_send_reminders(self, reminder_type: str = None) -> Dict[str, Any]:
        ...
        try:
            reminder_types = [reminder_type] if reminder_type else ["3days", "1day", "same_day"]
            results = {}
...
def get_reminder_scheduler_service(
    usage_service: UsageLimitService = None,
