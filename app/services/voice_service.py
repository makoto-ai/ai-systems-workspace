...
import logging
try:
    from core.voicevox import VoicevoxClient, VoicevoxConnectionError, EmotionParams, Speaker
    from core.speakers import VOICEVOX_SPEAKER_MAPPING
except ImportError:
    from app.core.voicevox import VoicevoxClient, VoicevoxConnectionError, EmotionParams, Speaker
...
class VoiceService:
    ...
    def __new__(cls, host: str = "localhost", port: str = "50021", test_mode: bool = False):
    ...
    def __init__(self, host: str = "localhost", port: str = "50021", test_mode: bool = False):
    ...
    async def synthesize_voice(
        ...
        try:
            ...
            if self.test_mode:
                ...
                # 実際のWAVファイルの最小構造を模擬
                wav_header = b'RIFF' + b'\x00' * 4 + b'WAVE' + b'fmt ' + b'\x00' * 16 + b'data' + b'\x00' * 4
    ...
    async def get_speakers(self) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            ...
            if self.test_mode:
                return {
                    "1": {"id": 1, "name": "Test Speaker 1", "description": "Test speaker for unit tests"},
                    "2": {"id": 2, "name": "Test Speaker 2", "description": "Test speaker for unit tests"},
                    "3": {"id": 3, "name": "Test Speaker 3", "description": "Test speaker for unit tests"},
                }
    ...
    async def validate_speaker_id(self, speaker_id: int) -> bool:
        ...
        try:
            ...
            if self.test_mode:
                ...
                logger.info(f"Test mode: Speaker ID {speaker_id} is {'valid' if is_valid else 'invalid'}")
                return is_valid
            
            speakers = await self.get_speakers()
            logger.info(f"Available speakers: {list(speakers.keys())}")
            is_valid = str(speaker_id) in speakers
            logger.info(f"Speaker ID {speaker_id} is {'valid' if is_valid else 'invalid'}")
            return is_valid
        except Exception as e:
