...
class PrivacyAwareContextService:
    ...
    def __init__(self, privacy_mode: PrivacyMode = PrivacyMode.STRICT):
        ...
        logger.info(f"Privacy-Aware Context Service initialized (mode: {privacy_mode.value})")
    ...
    async def update_context(
        ...
        try:
            ...
            if len(context.context_history) > self.max_context_length:
                context.context_history = context.context_history[-self.max_context_length:]
            ...
            # パターン識別
            context.identified_patterns = await self._identify_conversation_patterns(context)
            ...
            return {
                ...
                "engagement_trend": context.engagement_trend[-1] if context.engagement_trend else 0.5,
    ...
    async def _anonymize_content(self, content: str) -> str:
        ...
        try:
            ...
            # 会社名のパターンを除去
            anonymized = re.sub(r'(株式会社|有限会社|合同会社|[A-Za-z]+(?:株式会社|Corp|Inc|Ltd))', '[COMPANY]', anonymized)
            ...
            # 住所情報を除去
            anonymized = re.sub(r'[都道府県市区町村]{2,}[0-9一-九十百千万-]+', '[ADDRESS]', anonymized)
    ...
    async def _extract_context_features(
        ...
        try:
            ...
            # エンゲージメントレベル
            features["engagement_level"] = await self._calculate_engagement_level(user_input)
            
            # 営業ステージ推定
            features["sales_stage"] = await self._estimate_sales_stage(user_input, ai_response)
    ...
    async def _categorize_topic(self, content: str) -> str:
        ...
        try:
            ...
            # 営業関連トピック
            if any(word in content_lower for word in ["価格", "料金", "費用", "コスト"]):
                return "pricing"
            elif any(word in content_lower for word in ["機能", "特徴", "仕様", "スペック"]):
                return "features"
            elif any(word in content_lower for word in ["セキュリティ", "安全", "保護"]):
                return "security"
            elif any(word in content_lower for word in ["導入", "実装", "開始", "スケジュール"]):
                return "implementation"
            elif any(word in content_lower for word in ["サポート", "支援", "メンテナンス"]):
                return "support"
    ...
    async def _calculate_engagement_level(self, content: str) -> float:
        ...
        try:
            ...
            # ポジティブな指標
            positive_indicators = ["はい", "そうですね", "いいですね", "興味", "検討", "教えて"]
    ...
    async def _estimate_sales_stage(self, user_input: str, ai_response: str) -> str:
        ...
        try:
            ...
            if any(word in content_lower for word in ["初めて", "紹介", "概要", "どんな"]):
                return "prospecting"
            elif any(word in content_lower for word in ["課題", "問題", "困って", "必要"]):
                return "needs_assessment"
            elif any(word in content_lower for word in ["提案", "プラン", "見積", "価格"]):
                return "proposal"
            elif any(word in content_lower for word in ["心配", "不安", "懸念", "でも"]):
                return "objection_handling"
            elif any(word in content_lower for word in ["決めたい", "進めたい", "契約", "導入"]):
                return "closing"
            else:
    ...
    async def _calculate_sales_momentum(self, context: SessionContext) -> str:
        ...
        try:
            ...
            # 直近の傾向を分析
            recent_engagement = context.engagement_trend[-3:] if len(context.engagement_trend) >= 3 else context.engagement_trend
            
            if not recent_engagement:
                return "neutral"
            
            avg_engagement = sum(recent_engagement) / len(recent_engagement)
            trend = recent_engagement[-1] - recent_engagement[0] if len(recent_engagement) > 1 else 0
    ...
    async def _identify_conversation_patterns(self, context: SessionContext) -> List[str]:
        """会話パターンを識別"""
        try:
            patterns = []
    ...
    async def _get_topic_continuity_suggestions(self, current_topic: str, user_input: str) -> List[str]:
        """トピック継続性の提案"""
        try:
            suggestions = []
    ...
    async def _get_engagement_suggestions(self, engagement_trend: List[float]) -> List[str]:
        """エンゲージメント向上の提案"""
        try:
    ...
    async def _get_sales_stage_suggestions(self, sales_momentum: str, patterns: List[str]) -> List[str]:
        """営業ステージ適応の提案"""
        try:
            suggestions = []
    ...
    def _calculate_suggestion_confidence(self, context: SessionContext) -> float:
        ...
        try:
            ...
            # エンゲージメント傾向による調整
            if context.engagement_trend:
                avg_engagement = sum(context.engagement_trend) / len(context.engagement_trend)
    ...
    async def _periodic_cleanup(self) -> None:
        ...
        try:
            while True:
                ...
                timeout_delta = timedelta(minutes=self.privacy_config["session_timeout_minutes"])
        ...
        try:
            ...
            return {
                ...
                "summary": {
                    ...
                    "main_topics": list(set([turn.get("topic_category") for turn in context.context_history if turn.get("topic_category")])),
                    "current_topic": context.current_topic,
                    "sales_momentum": context.sales_momentum,
                    "average_engagement": sum(context.engagement_trend) / len(context.engagement_trend) if context.engagement_trend else 0.5,
                    "identified_patterns": context.identified_patterns,
                    "next_best_actions": context.next_best_actions
                },
