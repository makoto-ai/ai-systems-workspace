...
class SwallowTextService:
    ...
    def __init__(self, model_name: str = "tokyotech-llm/Llama-3.1-Swallow-8B-Instruct-v0.3"):
    ...
    def _generate_response(self, prompt: str, max_length: int = 1024) -> str:
        ...
        try:
            ...
            # チャット形式のプロンプト作成
            messages = [
                {"role": "system", "content": "あなたは営業分析の専門家です。正確で有用な分析を提供してください。"},
...
"""
    ...
    async def process_customer_document(self, text: str, document_type: str) -> TextAnalysisResult:
        """顧客ドキュメントの処理"""
        if not self.is_loaded:
            await self.load_model()
...
"""
    ...
    async def process_large_document_for_training(
        self, 
        text: str, 
        document_type: str = "sales_document",
        max_chunks: int = 20
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            return {
                ...
                "training_data": {
                    "structured_knowledge": self._extract_structured_knowledge(processed_chunks),
                ...
                "quality_metrics": {
                    ...
                    "average_importance": sum(c.get("importance_score", 0) for c in processed_chunks) / len(processed_chunks),
    ...
    def _select_important_chunks(self, chunks: List[str], max_chunks: int) -> List[str]:
        ...
        for chunk in chunks:
            ...
            # キーワードベースのスコアリング
            important_keywords = [
                '営業', '販売', '顧客', '商品', '価格', '提案', '契約', '成約',
                '課題', '解決', '効果', '利益', '競合', '差別化', 'ROI', 'KPI'
            ]
...
"""
    ...
    async def _create_document_summary(self, processed_chunks: List[Dict[str, Any]]) -> Dict[str, Any]:
        """文書全体の要約作成"""
        # 重要なポイントを集約
        all_key_points = []
        ...
        return {
            ...
            "document_structure": {
                "high_importance_chunks": len([c for c in processed_chunks if c.get("importance_score", 0) > 0.8]),
                "medium_importance_chunks": len([c for c in processed_chunks if 0.5 < c.get("importance_score", 0) <= 0.8]),
                "low_importance_chunks": len([c for c in processed_chunks if c.get("importance_score", 0) <= 0.5])
            }
        }
    
    def _extract_structured_knowledge(self, processed_chunks: List[Dict[str, Any]]) -> Dict[str, Any]:
        """構造化された知識を抽出"""
        return {
            "product_features": [],
            "pricing_info": [],
    ...
    def _generate_qa_pairs(self, processed_chunks: List[Dict[str, Any]]) -> List[Dict[str, str]]:
        """Q&Aペアを生成"""
        qa_pairs = []
    ...
    def _extract_sales_scenarios(self, processed_chunks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """営業シナリオを抽出"""
        scenarios = []
    ...
    async def process_high_quality_training_data(
        ...
        project_name: str = "sales_ai_training"
    ) -> Dict[str, Any]:
        """
        ...
        """
        try:
            logger.info(f"Starting high-quality processing for {len(documents)} documents")
            ...
            # Phase 4: 統合知識ベースの構築
            knowledge_base = await self._build_integrated_knowledge_base(cross_referenced)
            ...
            return {
                ...
                "ai_training_data": {
                    ...
                    "conversation_patterns": knowledge_base.get("conversation_patterns", []),
    ...
    async def _analyze_project_structure(self, documents: List[Dict[str, Any]]) -> Dict[str, Any]:
        """プロジェクト全体の構造を分析"""
        document_types = {}
        ...
        return {
            ...
            "complexity_score": len(document_types) * 0.2 + (total_content_length / 1000000) * 0.8
        }
    
    async def _process_by_priority(self, documents: List[Dict[str, Any]]) -> Dict[str, Any]:
        """優先度に基づく段階的処理"""
        # 優先度でソート（1が最高優先度）
        ...
        for doc in sorted_docs:
            ...
            if priority <= 2:
                # 高優先度: 詳細分析
                analysis = await self._detailed_analysis(content, doc.get("type", "general"))
                ...
                # 中優先度: 標準分析
                analysis = await self._standard_analysis(content, doc.get("type", "general"))
                ...
                # 低優先度: 基本分析
                analysis = await self._basic_analysis(content, doc.get("type", "general"))
    ...
    async def _analyze_chunk_detailed(self, chunk: str, doc_type: str, chunk_index: int) -> Dict[str, Any]:
        """チャンクの詳細分析"""
        prompt = f"""
...
"""
    ...
    async def _analyze_cross_references(self, priority_processed: Dict[str, Any]) -> Dict[str, Any]:
        """文書間の関連性分析"""
        cross_references = {
                ...
                if common_concepts:
                    ...
                    for concept in common_concepts:
                        ...
                        cross_references["product_feature_mapping"][concept].append({
                            "source_doc": doc_analysis.get("original_doc", {}).get("type", "unknown"),
                            "related_content": doc_concepts
                        })
        
        return cross_references
    
    async def _build_integrated_knowledge_base(self, cross_referenced: Dict[str, Any]) -> Dict[str, Any]:
        """統合知識ベースの構築"""
        knowledge_base = {
            "structured_data": {
                "products": {},
        ...
        for concept, references in cross_referenced.get("product_feature_mapping", {}).items():
    ...
    async def _validate_and_optimize(self, knowledge_base: Dict[str, Any]) -> Dict[str, Any]:
        """品質検証と最適化"""
        quality_metrics = {
        ...
        # 完全性チェック
        required_sections = ["products", "services", "pricing", "competitors", "customers"]
        filled_sections = sum(1 for section in required_sections 
    ...
    async def _analyze_chunk_standard(self, chunk: str, doc_type: str, chunk_index: int) -> Dict[str, Any]:
        """チャンクの標準分析"""
        prompt = f"""
...
"""
    ...
    async def _analyze_chunk_basic(self, chunk: str, doc_type: str, chunk_index: int) -> Dict[str, Any]:
        """チャンクの基本分析"""
        prompt = f"""
