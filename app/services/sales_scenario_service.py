...
class SalesScenarioService:
    ...
    async def process_scenario(
        ...
        try:
            ...
            # 応答生成
            response_text = self._generate_response(user_input, context, scenario_analysis)
            
            # ステージ進行の判定
            stage_progression = self._determine_stage_progression(context, scenario_analysis)
            
            # 推奨アクションの生成
            recommended_actions = self._generate_recommended_actions(context, scenario_analysis)
            ...
            return ScenarioResponse(
                ...
                scenario_analysis={"intent": "unknown", "sentiment": "neutral", "confidence": 0.0},
    ...
    def _analyze_intent(self, user_input: str) -> str:
        ...
        if any(keyword in user_input_lower for keyword in ["料金", "価格", "費用", "値段", "コスト"]):
            return "pricing_inquiry"
        elif any(keyword in user_input_lower for keyword in ["機能", "できること", "特徴"]):
    ...
    def _detect_intent(self, user_input: str) -> str:
        ...
        if any(keyword in user_input_lower for keyword in ["こんにちは", "はじめまして", "お疲れ様"]):
            return "greeting"
        elif any(keyword in user_input_lower for keyword in ["料金", "価格", "費用", "いくら"]):
            return "pricing_inquiry"
        elif any(keyword in user_input_lower for keyword in ["機能", "できること", "特徴", "教えて"]):
            return "feature_inquiry"
        elif any(keyword in user_input_lower for keyword in ["検討", "考える", "判断", "したい"]):
            return "decision"
        elif any(keyword in user_input_lower for keyword in ["高い", "高すぎ", "問題", "困る"]):
            return "objection"
        else:
            return "general_inquiry"
    ...
    def _detect_buying_signals(self, user_input: str) -> List[str]:
        ...
        if any(keyword in user_input.lower() for keyword in ["導入", "始める", "購入", "契約"]):
        ...
        if any(keyword in user_input.lower() for keyword in ["いつ", "時期", "スケジュール"]):
            signals.append("timeline_interest")
        if any(keyword in user_input.lower() for keyword in ["急い", "急ぎ", "至急", "早急"]):
    ...
    def _detect_objection_type(self, user_input: str) -> str:
        ...
        if any(keyword in user_input_lower for keyword in ["高い", "高額", "価格", "費用", "料金"]):
            return "price"
        elif any(keyword in user_input_lower for keyword in ["時間", "忙しい", "今は", "後で"]):
            return "timing"
        elif any(keyword in user_input_lower for keyword in ["上司", "決定", "承認", "権限"]):
            return "authority"
        elif any(keyword in user_input_lower for keyword in ["必要", "要らない", "不要", "わからない"]):
            return "need"
        elif any(keyword in user_input_lower for keyword in ["信頼", "心配", "不安", "疑問"]):
            return "trust"
        elif any(keyword in user_input_lower for keyword in ["他社", "競合", "比較", "検討中"]):
            return "competition"
        else:
            return "general"
    ...
    def _detect_objections(self, user_input: str) -> List[Dict[str, Any]]:
        ...
        if any(keyword in user_input_lower for keyword in ["時間", "忙しい", "タイミング"]):
            objections.append({"type": "timing", "details": "時期に関する反対意見"})
        if any(keyword in user_input_lower for keyword in ["上司", "決定権", "承認"]):
            objections.append({"type": "authority", "details": "決定権に関する反対意見"})
        ...
        if any(keyword in user_input_lower for keyword in ["他社", "競合", "比較"]):
            objections.append({"type": "competition", "details": "競合に関する反対意見"})
    ...
    def _extract_bant_updates(
        ...
        # Budget（予算）の抽出
        if any(keyword in user_input.lower() for keyword in ["予算", "費用", "万円", "円"]):
            # 具体的な金額が言及されている場合は qualified
            if any(keyword in user_input.lower() for keyword in ["万円", "円", "程度", "くらい"]):
                updates["budget"] = {"status": "qualified", "details": "予算が具体的に示されました"}
            else:
                updates["budget"] = {"status": "interested", "details": "予算に関する興味"}
        
        # Authority（決定権）の抽出
        if any(keyword in user_input.lower() for keyword in ["決定", "上司", "承認"]):
            updates["authority"] = {"status": "mentioned", "details": "決定権に関する言及"}
        
        # Need（ニーズ）の抽出
        if any(keyword in user_input.lower() for keyword in ["必要", "課題", "問題", "業務効率", "改善"]):
            updates["need"] = {"status": "identified", "details": "ニーズが特定されました"}
        
        # Timeline（時期）の抽出
        if any(keyword in user_input.lower() for keyword in ["いつ", "時期", "急い", "来月", "来週", "今月", "今年", "来年"]):
