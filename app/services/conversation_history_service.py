...
class ConversationHistoryService:
    ...
    def __init__(self, storage_backend: str = "memory"):
        ...
        logger.info(f"Conversation history service initialized with {storage_backend} backend")
    ...
    async def create_session(
        ...
        logger.info(f"Created conversation session {session_id} for customer {customer_profile.customer_id}")
    ...
    async def add_turn(
        ...
        turn = ConversationTurn(
            ...
            processing_time=analysis_data.get("processing_time") if analysis_data else None
        )
    ...
    async def update_bant_status(
        ...
        logger.info(f"Updated BANT status for session {session_id}, score: {session.bant_status.score}")
        return True
    ...
    def _generate_conversation_summary(self, session: ConversationSession) -> Dict[str, Any]:
    ...
    def _calculate_session_metrics(self, session: ConversationSession) -> Dict[str, Any]:
        ...
        processing_times = [t.processing_time for t in session.turns if t.processing_time]
    ...
    def _assess_engagement_level(self, session: ConversationSession) -> str:
        ...
        for turn in customer_turns:
            ...
            if any(word in content for word in ["はい", "そうですね", "いいですね", "興味", "検討"]):
...
def get_conversation_history_service(storage_backend: str = "memory") -> ConversationHistoryService:
    ...
    if _conversation_history_service is None:
        _conversation_history_service = ConversationHistoryService(storage_backend=storage_backend)
