...
class LanguageAnalysisService:
    ...
    def initialize_language_patterns(self):
        ...
        self.keigo_patterns = {
            ...
            "teineigo": {  # 丁寧語
                "patterns": [r"です", r"ます", r"ございます", r"であります"],
        ...
        self.business_expressions = {
            "opening": ["お忙しい中", "貴重なお時間", "恐れ入ります", "失礼いたします"],
            "consideration": [
                "ご検討",
                "ご相談",
                "ご質問",
            ...
            "gratitude": ["ありがとうございます", "感謝申し上げます", "恐縮です"],
            "apology": ["申し訳ございません", "恐れ入ります", "失礼いたしました"],
        }

        # 不適切表現（営業で避けるべき）
        self.inappropriate_expressions = {
            "too_casual": ["やばい", "すげー", "マジで", "ぶっちゃけ", "てか", "っす"],
            "unclear": ["なんか", "とか", "みたいな", "かも", "多分", "たぶん"],
            "negative": ["無理", "できません", "ダメ", "困ります", "面倒"],
            "pushy": ["絶対", "必ず", "間違いなく", "確実に", "100%"],
        }
    ...
    async def analyze_language_quality(
        self, text: str, context: str = "sales", customer_level: str = "business"
    ) -> Dict[str, Any]:
        """
        言葉遣いの総合分析
        ...
        try:
            ...
            return {
                ...
                "context_analysis": self._analyze_context_appropriateness(
                    text, context
                ),
    ...
    def _analyze_politeness(self, text: str) -> PolitenessAnalysis:
        ...
        politeness_ratio = teineigo_count / max(
            text_length / 10, 1
        )  # 10文字あたりの丁寧語
    ...
    def _evaluate_business_appropriateness(
        self, text: str, customer_level: str
    ) -> Dict[str, Any]:
        """ビジネス適切性の評価"""
        ...
        for category, expressions in self.business_expressions.items():
            ...
            business_score += (category_score / len(expressions)) * weights.get(
                category, 0.25
            )
        ...
        return {
            ...
            "recommendations": self._get_customer_level_recommendations(
                customer_level, business_score
            ),
        }

    def _generate_language_improvements(
        self, text: str, issues: List[LanguageIssue]
    ) -> Dict[str, Any]:
        """言語改善提案の生成"""
    ...
    def _analyze_context_appropriateness(
        ...
        # 必要要素のチェック
        required_score = self._check_required_elements(
            text, requirements["required_elements"]
        )

        # 避けるべき要素のチェック
        avoided_score = self._check_avoided_elements(
            text, requirements["avoided_elements"]
        )
        ...
        return {
            ...
            "recommendations": self._get_context_recommendations(
                context, required_score, avoided_score
            ),
        }
    ...
    def _check_keigo_appropriateness(
        self, keigo_usage: Dict[str, Any]
    ) -> Dict[str, str]:
        """敬語の適切性をチェック"""
        appropriateness = {}
    ...
    def _get_customer_level_recommendations(
        ...
        recommendations = {
            ...
            "casual": ["親しみやすさを保ちつつ、基本的な丁寧さを守ってください"],
        }
    ...
    def _get_context_recommendations(
        self, context: str, required: float, avoided: float
    ) -> List[str]:
        """文脈別推奨事項"""
        recommendations = []
    ...
    def _evaluate_tone_appropriateness(self, tone_scores: Dict[str, float]) -> float:
        ...
        weighted_score = sum(
            tone_scores[tone] * weight for tone, weight in weights.items()
        )
