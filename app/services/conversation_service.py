...
try:
    ...
    from services.privacy_aware_context_service import get_privacy_aware_context_service, PrivacyMode
    ...
    from app.services.privacy_aware_context_service import get_privacy_aware_context_service, PrivacyMode
...
class ConversationService:
    ...
    def __init__(self):
        ...
        logger.info("Conversation service initialized with privacy-aware context understanding")
    ...
    async def process_voice_conversation(
        ...
        try:
            ...
            speech_service = get_speech_service(model_size="medium")  # High accuracy model for better recognition
            ...
            if context_suggestions.get("suggestions"):
                ...
                context_info += f"- 提案: {', '.join(context_suggestions['suggestions'][:2])}\n"
                enhanced_prompt += context_info
            
            ai_response = await self.groq_service.sales_analysis(enhanced_prompt)
            
            response_text = ai_response.get("response", "申し訳ございません、もう一度お話しください。")
            ...
            asyncio.create_task(
                self.history_service.add_turn(
                    ...
                    analysis_data={"confidence": transcription_result.get("confidence", 0.8)}
                )
            )
            ...
            result = {
                ...
                "context": {
                    ...
                    "suggestions_available": len(context_suggestions.get("suggestions", [])) > 0,
            ...
            logger.info(f"Voice conversation processed in {processing_time:.2f}s with privacy protection")
            return result
    ...
    async def initialize_voice_service(self, host: str = "localhost", port: str = "50021"):
    ...
    async def handle_objection(
        ...
        try:
            ...
            if context_suggestions.get("suggestions"):
                enhanced_prompt += f"文脈提案: {', '.join(context_suggestions['suggestions'][:2])}\n"
            
            # AI分析
            ai_response = await self.groq_service.sales_analysis(enhanced_prompt)
            response_text = ai_response.get("response", "ご懸念をお聞かせいただき、ありがとうございます。")
            ...
            return {
                ...
                "objection_handling": {
                    ...
                    "strategy": ai_response.get("handling_strategy", "empathetic_response"),
                    "confidence": 0.8
                },
                "output": {
                    "text": response_text,
        ...
        try:
            ...
            bant_analysis = {
                "bant_status": {
                    ...
                    "need": {"status": "qualified" if summary.get("identified_patterns") else "exploring"},
    ...
    async def get_session_analytics(self, session_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            analytics = {
                ...
                "concerns": [],
                "engagement_insights": {
                    "engagement_level": "medium" if summary.get("average_engagement", 0.5) > 0.5 else "low",
