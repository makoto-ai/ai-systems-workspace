...
class FriendlinessAnalyzer:
    ...
    def initialize_patterns(self):
        ...
        self.warmth_expressions = {
            "empathy": {
                "patterns": [
                    "お気持ちお察しします", "よくわかります", "そうですよね", 
                    "なるほど", "確かに", "おっしゃる通り", "同感です"
                ],
                "weight": 0.3
            },
            "understanding": {
        ...
        self.empathy_expressions = {
            "acknowledgment": [
                "おっしゃる通り", "確かに", "そうですね", "なるほど",
                "同感です", "よくわかります", "理解できます"
            ],
            "validation": [
                "お気持ちお察しします", "ご心配ですね", "大変ですね",
                "お困りでしょう", "ご苦労されて", "お忙しい中"
            ],
            "support": [
                "お手伝いします", "サポートします", "一緒に考えましょう",
                "お任せください", "安心してください", "大丈夫です"
            ]
        }
        ...
        self.natural_expressions = {
            ...
            "conversational": [
                "お話しましょう", "相談しましょう", "話し合いましょう",
                "聞かせてください", "教えてください", "一緒に"
            ],
            "approachable": [
                "気軽に", "遠慮なく", "お気軽に", "いつでも",
                "何でも", "どんなことでも", "お声がけください"
            ]
        }
    ...
    def analyze_friendliness(
        ...
        overall_score = self._calculate_friendliness_score(
            warmth_score, distance_score, empathy_score, 
            naturalness_score, emotion_score
        )
        ...
        return FriendlinessAnalysis(
            ...
            warmth_indicators=warmth_indicators + empathy_indicators + emotion_indicators,
    ...
    def _analyze_empathy(self, text: str) -> Tuple[float, List[str]]:
        ...
        total_patterns = sum(len(patterns) for patterns in self.empathy_expressions.values())
        found_patterns = 0
    ...
    def _analyze_naturalness(self, text: str) -> Tuple[float, List[str]]:
        ...
        total_patterns = sum(len(patterns) for patterns in self.natural_expressions.values())
        found_patterns = 0
        ...
        naturalness_score = found_patterns / total_patterns if total_patterns > 0 else 0.0
    ...
    def _analyze_emotional_expression(self, text: str) -> Tuple[float, List[str]]:
        ...
        total_patterns = sum(len(patterns) for patterns in self.emotional_expressions.values())
        found_patterns = 0
    ...
    def _detect_friendliness_issues(
        ...
        for category, data in self.distance_expressions.items():
            ...
            if found_patterns:
                issues.append(FriendlinessIssue(
    ...
    def _calculate_balance_score(
        ...
        naturalness_diff = abs(naturalness - target_naturalness) / max(target_naturalness, 1.0)
        distance_diff = abs(distance - target_distance) / max(target_distance, 1.0) if target_distance > 0 else distance
    ...
    def _generate_recommendations(
        ...
        for issue in issues:
            if issue.issue_type == DistanceIssueType.LACK_OF_WARMTH:
                ...
                recommendations.append("丁寧さを保ちながら、より親しみやすい表現を使用してください")
        
        # 関係性別の推奨事項
        if customer_relationship == "new":
            recommendations.append("初回のお客様には親しみやすさと信頼性のバランスを心がけてください")
        elif customer_relationship == "long_term":
            recommendations.append("長期のお客様には信頼関係を活かした温かみのある表現を使用してください")
