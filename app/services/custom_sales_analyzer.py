...
class CustomSalesAnalyzer:
    ...
    def _evaluate_rapport_building(self, text: str) -> Dict[str, Any]:
        ...
        polite_score = sum(1 for expr in polite_expressions if expr in text) / len(
            polite_expressions
        )
        ...
        return {
            "score": total_score,
            "breakdown": {"keyword_usage": keyword_score, "politeness": polite_score},
            "rating": self._convert_to_rating(total_score, criteria),
            "suggestions": self._generate_rapport_suggestions(total_score),
        }

    def _evaluate_needs_discovery(
        self, text: str, conversation_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """ニーズ発見力の評価"""
    ...
    def _evaluate_value_proposition(self, text: str) -> Dict[str, Any]:
        ...
        benefit_score = sum(1 for keyword in benefit_keywords if keyword in text) / len(
            benefit_keywords
        )
        ...
        concrete_score = min(
            sum(1 for indicator in concrete_indicators if indicator in text) / 2, 1.0
        )

        # ROI言及
        roi_indicators = ["投資", "リターン", "効果", "削減", "節約"]
        roi_score = min(
            sum(1 for indicator in roi_indicators if indicator in text) / 3, 1.0
        )
    ...
    def _evaluate_objection_handling(
        ...
        acknowledgment_score = sum(
            1 for pattern in acknowledgment_patterns if pattern in text
        ) / len(acknowledgment_patterns)

        # 解決策提示
        solution_indicators = ["提案", "解決", "対応", "方法", "できます"]
        solution_score = min(
            sum(1 for indicator in solution_indicators if indicator in text) / 3, 1.0
        )
    ...
    def _analyze_industry_specific_aspects(
        self, conversation_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """業界特化の分析"""
        # 業界固有の要素を分析
    ...
    def _analyze_coaching_perspective(
        self, conversation_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """コーチング視点での分析"""
        # 個人のコーチング方針に基づく分析
    ...
    def _analyze_emotional_intelligence(
        ...
        return {
            ...
            "overall_eq": (emotion_recognition + emotion_regulation + social_awareness)
            / 3,
        }
    ...
    def _calculate_overall_assessment(
        self, basic_analysis: Dict[str, Any], custom_analysis: Dict[str, Any]
    ) -> Dict[str, Any]:
        """総合評価の計算"""
        # 基本スコアの重み付け平均
        basic_scores = [
            basic_analysis["rapport_building"]["score"]
            * self.default_criteria["rapport_building"]["weight"],
            basic_analysis["needs_discovery"]["score"]
            * self.default_criteria["needs_discovery"]["weight"],
            basic_analysis["value_proposition"]["score"]
            * self.default_criteria["value_proposition"]["weight"],
            basic_analysis["objection_handling"]["score"]
            * self.default_criteria["objection_handling"]["weight"],
        ]
    ...
    def _generate_improvement_suggestions(
        ...
        if "emotional_intelligence" in custom_analysis:
            ...
            if eq_score < 0.7:
                suggestions.append(
                    "感情的知性の向上：相手の感情をより敏感に察知し、適切に対応する練習をしましょう"
                )
    ...
    def _evaluate_assertiveness(self, text: str) -> float:
        ...
        score = sum(1 for indicator in assertive_indicators if indicator in text) / len(
    ...
    def _evaluate_empathy(self, text: str) -> float:
        ...
        score = sum(1 for indicator in empathy_indicators if indicator in text) / len(
            empathy_indicators
        )
        return min(score, 1.0)
    ...
    def _evaluate_logical_thinking(self, text: str) -> float:
        ...
        score = sum(1 for indicator in logical_indicators if indicator in text) / len(
            logical_indicators
        )
        return min(score, 1.0)
    ...
    def _analyze_it_industry_aspects(
        self, conversation_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """IT業界特化分析"""
        return {"industry_focus": "IT", "technical_appropriateness": 0.8}

    def _analyze_manufacturing_aspects(
        self, conversation_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """製造業特化分析"""
        return {"industry_focus": "Manufacturing", "qcd_focus": 0.7}

    def _analyze_general_industry_aspects(
        self, conversation_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """一般業界分析"""
