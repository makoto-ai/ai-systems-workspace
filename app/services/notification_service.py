...
class MultiChannelNotificationService:
    ...
    async def _send_slack(
        self, message: NotificationMessage, config: NotificationConfig
    ) -> Dict[str, Any]:
        """Slack通知送信"""
            ...
            "attachments": [
                {
                    ...
                    "fields": [
                        {"title": "ユーザー", "value": message.user_id, "short": True},
                        {
                            "title": "リマインダータイプ",
        ...
        try:
            async with self.session.post(
                config.webhook_url, json=slack_message
            ) as response:
    ...
    async def _send_discord(
            ...
            "embeds": [
                {
                    ...
                    "color": int(
                        self._get_color_for_reminder(
                            message.reminder_type, format="int"
                        )
                    ),
                    "timestamp": datetime.now().isoformat(),
                    "footer": {"text": "営業ロールプレイシステム"},
                    "fields": [
                        {"name": "ユーザー", "value": message.user_id, "inline": True},
                        {
                            "name": "タイプ",
                            "value": message.reminder_type,
                            "inline": True,
        ...
        try:
            async with self.session.post(
                config.webhook_url, json=discord_message
            ) as response:
                if response.status == 204:  # Discord returns 204 on success
                    ...
                    return {
                        "success": False,
                        "error": f"Discord API error: {error_text}",
                    }
        except Exception as e:
...
📋 タイプ: {message.reminder_type}
        ...
        payload = {
            ...
            "disable_web_page_preview": config.additional_config.get(
                "disable_web_page_preview", True
            ),
        }
        ...
        if message.action_url:
            payload["reply_markup"] = {
                "inline_keyboard": [
                    [{"text": "🚀 ロールプレイ開始", "url": message.action_url}]
                ]
            }
    ...
    async def _send_teams(
        ...
        teams_message = {
            ...
            "summary": config.additional_config.get(
                "summary", "営業ロールプレイリマインダー"
            ),
            "sections": [
                {
                    ...
                    "text": message.content,
                    "facts": [
                        {"name": "リマインダータイプ", "value": message.reminder_type},
                        {
                            "name": "送信時刻",
            ...
            teams_message["potentialAction"] = [
                {
                    "@type": "OpenUri",
                    "name": "ロールプレイ開始",
                    "targets": [{"os": "default", "uri": message.action_url}],
                }
            ]

        try:
            async with self.session.post(
                config.webhook_url, json=teams_message
            ) as response:
                if response.status == 200:
    ...
    async def _send_line(
        ...
        # コスト警告ログ
        logger.warning(
            "LINE API は従量課金です。月1000通まで無料、以降は課金されます。"
        )
            ...
            line_message["messages"] = [
                {
                    ...
                    "contents": {
                        ...
                        "body": {
                            ...
                            "layout": "vertical",
                            "contents": [
                                {"type": "text", "text": message.content, "wrap": True}
                            ],
                        },
                        "footer": {
                            "type": "box",
        ...
        try:
            async with self.session.post(
                url, json=line_message, headers=headers
            ) as response:
                if response.status == 200:
    ...
    async def _send_sms(
        ...
        if not config.api_token or not config.phone_number:
            return {
                "success": False,
                "error": "SMS API token or phone number not configured",
            }

        # コスト警告ログ
        logger.warning(
            f"SMS API は従量課金です。{config.additional_config.get('cost_per_message', '約5-10円/通')}"
        )
    ...
    async def _send_twilio_sms(
        self, content: str, config: NotificationConfig
    ) -> Dict[str, Any]:
        """Twilio SMS送信"""
    ...
    async def send_multi_channel(
        self, message: NotificationMessage, channels: List[str]
    ) -> Dict[str, Any]:
        """
        複数チャンネルに同時送信
        ...
        for channel in channels:
            if (
                channel in self.channels_config
                and self.channels_config[channel].enabled
            ):
