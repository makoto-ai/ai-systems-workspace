    ...
    def __post_init__(self):
        if self.recent_sessions is None:
            ...
            if isinstance(self.recent_sessions[0], dict):
                self.recent_sessions = [RoleplaySession(**session) for session in self.recent_sessions]
...
class UsageLimitService:
    ...
    def __init__(self, storage_path: str = "data/usage_limits.json"):
        ...
        logger.info(f"Usage limit service initialized with storage: {self.storage_path}")
    ...
    async def can_process_video(self, user_id: str, video_duration_minutes: int) -> Dict[str, Any]:
        """
        ...
        remaining_minutes = self.max_video_processing_minutes - user_stats.video_processing_minutes_used
    ...
    async def consume_video_processing(self, user_id: str, video_duration_minutes: int) -> Dict[str, Any]:
        """
        ...
        can_process_result = await self.can_process_video(user_id, video_duration_minutes)
        ...
        logger.info(f"User {user_id} consumed {video_duration_minutes}min video processing + 1 roleplay session")
        ...
        return {
            ...
            "remaining_video_minutes": self.max_video_processing_minutes - user_stats.video_processing_minutes_used,
    ...
    async def get_user_usage(self, user_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "video_processing": {
                ...
                "minutes_remaining": self.max_video_processing_minutes - user_stats.video_processing_minutes_used,
            ...
            "roleplay_sessions": {
                ...
                "used": self.initial_roleplay_sessions - user_stats.roleplay_sessions_remaining
            },
            "period_info": {
                "last_reset": user_stats.last_reset_date,
                "reset_period_days": self.reset_period_days,
                "next_reset": self._calculate_next_reset_date(user_stats.last_reset_date)
            },
    ...
    async def record_roleplay_session(
        ...
        try:
            ...
            if improvement_points:
                ...
                user_stats.recent_improvement_points = user_stats.recent_improvement_points[-5:]  # Keep last 5
            
            # Calculate consecutive days
            user_stats.consecutive_days = await self._calculate_consecutive_days(user_id)
    ...
    async def get_users_for_reminder(self, reminder_type: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"Found {len(reminder_users)} users for {reminder_type} reminder")
    ...
    async def get_user_roleplay_stats(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "recent_sessions": [asdict(session) for session in user_stats.recent_sessions],
                ...
                # 突きつけ用の詳細データ
                "confrontation_data": self._generate_confrontation_data(user_stats, days_since_last)
            }
    ...
    def _generate_confrontation_data(self, user_stats: UsageStats, days_since_last: int) -> Dict[str, Any]:
        """
        サボり突きつけ用のデータ生成
        ...
        # サボり機能が無効の場合は基本情報のみ
        shame_enabled = user_stats.reminder_settings.enable_shame_system if user_stats.reminder_settings else False
        ...
        if days_since_last == 0:
            ...
            slacking_message = "今日もロールプレイを実行しています！素晴らしい継続力です。"
        ...
        # 過去の実績
        total_sessions = user_stats.total_roleplay_sessions
        best_streak = user_stats.consecutive_days if user_stats.consecutive_days > 0 else 0
        
        # 損失計算
        potential_sessions_lost = max(0, days_since_last - 1)  # 本来できたはずのセッション数
        ...
        if days_since_last > 3:
            harsh_messages.append(f"あなたは{days_since_last}日間、営業スキル向上を放置しています。")
        
        if potential_sessions_lost > 0:
            harsh_messages.append(f"この期間に{potential_sessions_lost}回のロールプレイができたはずです。")
            harsh_messages.append(f"約{investment_wasted}分の成長機会を無駄にしました。")
        
        if total_sessions > 0 and days_since_last > 7:
            harsh_messages.append(f"これまで{total_sessions}回のセッションを積み重ねたのに、その努力を台無しにしています。")
        
        if best_streak > 0 and days_since_last > 3:
            harsh_messages.append(f"過去の最大連続記録{best_streak}日を裏切る行為です。")
        
        if improvement_stagnation:
            harsh_messages.append("改善ポイントも記録されておらず、成長への意欲が見られません。")
        
        # 競合他社に負ける警告
        if days_since_last > 7:
            harsh_messages.append("競合他社の営業担当は毎日スキルを磨いています。あなたは確実に遅れをとっています。")
        ...
        return {
            ...
            "shame_metrics": {
                ...
                "productivity_loss_percentage": min(100, (days_since_last / 30) * 100),  # 30日で100%とする
                "skill_deterioration_risk": "高" if days_since_last > 7 else "中" if days_since_last > 3 else "低",
                "competitive_disadvantage": "深刻" if days_since_last > 14 else "危険" if days_since_last > 7 else "軽微"
            }
        }
    ...
    async def get_all_users_activity_report(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for user_id, user_stats in self.usage_stats.items():
                ...
                user_report = {
                    "user_id": user_id,
                    "user_name": user_stats.reminder_settings.user_name if user_stats.reminder_settings else user_id,
                    "last_roleplay_date": user_stats.last_roleplay_date,
                    "days_since_last": days_since_last,
                    "total_sessions": user_stats.total_roleplay_sessions,
                    "consecutive_days": user_stats.consecutive_days,
                    ...
                    "risk_level": confrontation_data["shame_metrics"]["competitive_disadvantage"]
                }
            ...
            slacking_users = len([u for u in all_users_report if u["days_since_last"] > 3])
            critical_users = len([u for u in all_users_report if u["days_since_last"] > 14])
            ...
            for user in all_users_report["users"]:
                if user["days_since_last"] > 7:  # 1週間以上サボっている
                    ...
                    if user_stats and user_stats.reminder_settings and user_stats.reminder_settings.enable_shame_system:
            ...
            return {
                ...
                "summary": {
                    ...
                    "slacking_rate": round((slacking_users / total_users * 100), 1) if total_users > 0 else 0
                },
    ...
    async def admin_add_sessions(self, user_id: str, additional_sessions: int) -> Dict[str, Any]:
