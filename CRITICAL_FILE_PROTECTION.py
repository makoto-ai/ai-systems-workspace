...
class CriticalFileProtector:
    ...
    def __init__(self, project_root: str = None):
        self.project_root = Path(
            project_root or "/Users/araimakoto/ai-driven/ai-systems-workspace"
        )
    ...
    def check_operation_safety(
        self, operation: str, target_paths: List[str]
    ) -> Dict[str, Any]:
        """操作の安全性をチェック"""
        if not self.config["protection_enabled"]:
    ...
    def _check_single_file(self, file_path: Path, results: Dict, operation: str):
        """単一ファイルの安全性チェック"""
        if self._is_critical_file(file_path) and not self._is_allowed_deletion(
            file_path
        ):
            results["safe"] = False
