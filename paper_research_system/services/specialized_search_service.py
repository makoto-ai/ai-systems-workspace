...
class SpecializedSearchService:
    ...
    async def specialized_search(
        ...
        for enhanced_query in critical_queries[:5]:  # 上位5クエリ
            papers = await self.base_search.search_papers(
                enhanced_query, max(1, max_results // len(critical_queries[:5]))
            )
    ...
    def _generate_domain_queries(self, base_query: str, domain: str) -> List[str]:
        """ドメイン特化クエリ生成"""
    ...
    def _apply_critical_thinking_mode(self, queries: List[str], mode: str) -> List[str]:
        """クリティカルシンキングモード適用"""
    ...
    def _apply_domain_filtering(
        self, papers: List[Paper], domain: str, mode: str
    ) -> List[Paper]:
        """ドメイン特化フィルタリング"""
        domain_config = self.domain_taxonomy.get(domain, {})
        mode_config = self.critical_thinking_modes.get(mode, {})
        ...
        for paper in papers:
            # ドメイン関連性スコア計算
            domain_score = self._calculate_domain_relevance(paper, domain_config)
            ...
            if (
                domain_score >= 0.1
                or mode_score >= 0.1
                or (domain_score + mode_score) >= 0.2
            ):
    ...
    def _calculate_domain_relevance(self, paper: Paper, domain_config: Dict) -> float:
        """ドメイン関連性スコア計算"""
        score = 0.0
        ...
        for category, concepts in domain_config.items():
            if category in ["name", "filters"] or not isinstance(concepts, list):
                continue
    ...
    def _calculate_mode_relevance(self, paper: Paper, mode_config: Dict) -> float:
        """クリティカルシンキングモード関連性"""
        score = 0.0
        text_to_check = f"{paper.title} {paper.abstract or ''}".lower()
        focus_terms = mode_config.get("search_focus", [])
        ...
        # フォーカス用語での一致度
        matches = sum(1 for term in focus_terms if term.lower() in text_to_check)
    ...
    def _rank_papers_by_mode(
        self, papers: List[Paper], mode: str, max_results: int
    ) -> List[Paper]:
        """モード別ランキング"""
        mode_config = self.critical_thinking_modes.get(mode, {})
        ...
        # カスタムスコア計算
        for paper in papers:
            paper.specialized_score = self._calculate_specialized_score(paper, mode)
