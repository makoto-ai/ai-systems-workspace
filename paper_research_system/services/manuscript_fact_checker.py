...
class ManuscriptFactChecker:
    ...
    def fact_check_claims(self, claims: List[ExtractedClaim]) -> List[FactCheckResult]:
        ...
        for claim in claims:
            ...
            if not search_successful:
                print(
                    f"🚨 全ての検索が失敗: {claim.content[:50]}... → ハルシネーション判定"
                )
                all_evidence = []
            ...
            recommendation = self._generate_recommendation(
                claim, all_evidence, is_hallucination
            )
    ...
    def generate_corrected_manuscript(
        self, original_manuscript: str, fact_check_results: List[FactCheckResult]
    ) -> str:
        """修正版原稿を生成"""
        ...
        for result in fact_check_results:
            if result.is_hallucination:
                ...
                corrected_manuscript = self._replace_maintaining_style(
                    corrected_manuscript, result.original_claim.content, replacement
                )
                ...
                corrected_manuscript = self._enhance_text_maintaining_style(
                    corrected_manuscript, result.original_claim.content, enhancement
                )
            ...
            if matches:
                if isinstance(matches[0], tuple):
                    ...
                    for match in matches:
                        combined_name = " ".join(
                            [name.strip() for name in match if name.strip()]
                        )
                        if combined_name:
    ...
    def _generate_search_queries(self, claim: ExtractedClaim) -> List[str]:
        ...
        if claim.researcher_name and claim.publication_year:
            ...
            print(
                f"🔍 具体的研究を検証: '{claim.researcher_name}' ({claim.publication_year}年)"
            )
            return queries
    ...
    def _detect_hallucination(self, claim: ExtractedClaim, evidence: List[Any]) -> bool:
        ...
        if claim.researcher_name and claim.publication_year:
            ...
            if not matching_papers:
                print(
                    f"📋 研究者'{claim.researcher_name}'年代'{claim.publication_year}'が一致する論文なし → ハルシネーション"
                )
                return True
            else:
                print(
                    f"✅ 研究者'{claim.researcher_name}'年代'{claim.publication_year}'の論文を確認: {len(matching_papers)}件"
                )
                return False
        ...
        if claim.researcher_name:
            ...
            if not found_researcher:
                print(
                    f"📋 研究者'{claim.researcher_name}'が見つからない → ハルシネーション"
                )
                return True
        ...
        if claim.publication_year:
            ...
            if not found_year:
                print(
                    f"📋 年代'{claim.publication_year}'の論文が見つからない → ハルシネーション"
                )
                return True
    ...
    def _calculate_verification_score(
        self, claim: ExtractedClaim, evidence: List[Any]
    ) -> float:
        """検証スコアを計算"""
        if not evidence:
            return 0.0
        ...
        score = 0.0
        total_citations = sum(
            getattr(paper, "citation_count", 0) or 0 for paper in evidence
        )
        ...
        recent_papers = [
            p
            for p in evidence
            if hasattr(p, "publication_year")
            and p.publication_year
            and int(p.publication_year) >= 2010
        ]
        if recent_papers:
            score += 0.3
    ...
    def _generate_recommendation(
        self, claim: ExtractedClaim, evidence: List[Any], is_hallucination: bool
    ) -> str:
        """推奨事項を生成"""
        if is_hallucination:
    ...
    def _create_evidence_based_replacement(self, result: FactCheckResult) -> str:
        ...
        if hasattr(best_paper, "citation_count") and best_paper.citation_count:
            if best_paper.citation_count > 1000:
                citation_info = (
                    f"（{best_paper.citation_count:,}回以上引用されている権威的研究）"
                )
        ...
        replacement = (
            f"{author_name}の{year}の研究{citation_info}によると、{core_claim}{ending}"
        )
        return replacement
    ...
    def _extract_core_claim(self, text: str) -> str:
        ...
        # 語尾を除去
        cleaned = re.sub(
            r"(?:そうです|です|ます|である|だ|と言われています)$", "", cleaned
        )
    ...
    def _extract_ending(self, text: str) -> str:
        ...
        ending_match = re.search(
            r"(そうです|です|ます|である|だ|と言われています)$", text
        )
        if ending_match:
            return ending_match.group(1)
        return "そうです"  # デフォルト

    def _replace_maintaining_style(
        self, text: str, original: str, replacement: str
    ) -> str:
        """文体を維持しながら置換"""
    ...
    def _enhance_credibility(self, result: FactCheckResult) -> str:
        ...
        best_paper = max(
            result.evidence_papers, key=lambda p: getattr(p, "citation_count", 0) or 0
        )
    ...
    def _enhance_text_maintaining_style(
        self, text: str, original: str, enhancement: str
    ) -> str:
        """文体を維持しながら情報を強化"""
        if not enhancement:
            return text
    ...
    def run_full_fact_check(self, manuscript: str) -> Dict[str, Any]:
        ...
        # 3. 修正版生成
        print("✏️ 修正版原稿を生成中...")
        corrected_manuscript = self.generate_corrected_manuscript(
            manuscript, fact_check_results
        )

        # 4. 結果をObsidianに保存
        self._save_results_to_obsidian(
            manuscript, fact_check_results, corrected_manuscript
        )

        # 5. 結果サマリー
        hallucination_count = sum(1 for r in fact_check_results if r.is_hallucination)
        low_confidence_count = sum(
            1 for r in fact_check_results if r.verification_score < 0.5
        )
    ...
    def _save_results_to_obsidian(
        self, original: str, results: List[FactCheckResult], corrected: str
    ):
        """結果をObsidianに保存"""
...
"""
    ...
    def _split_researcher_name(self, researcher_name: str) -> List[str]:
        ...
        return [
            part for part in parts if part and len(part) > 2
        ]  # 2文字以上の有効な姓のみ
...
def new_extract_researchers(self, sentence):
    ...
    # 重複除去し、andを含むものを先頭に
    unique = list(set(researchers))
    and_first = [r for r in unique if " and " in r] + [
        r for r in unique if " and " not in r
    ]
...
def new_detect_hallucination(self, claim, evidence):
    ...
    print(
        f"🔧 新規ハルシネーション判定: '{claim.researcher_name}' ({claim.publication_year})"
    )
    ...
    if claim.researcher_name and claim.publication_year:
        ...
        for i, paper in enumerate(evidence):
            ...
            if hasattr(paper, "publication_year") and paper.publication_year:
                try:
                    year_diff = abs(
                        int(paper.publication_year) - int(claim.publication_year)
                    )
                    year_match = year_diff <= 1
                    print(
                        f"  → 年代マッチ: {year_match} ({paper.publication_year} vs {claim.publication_year})"
                    )
                except (ValueError, TypeError):
                    print(f"  → 年代エラー")
                    pass
