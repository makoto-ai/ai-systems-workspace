...
class SafeRateLimitedSearchService:
    ...
    async def search_papers(
        ...
        Returns:
            統合された論文リスト
        """
        logger.info(f"安全な統合検索開始 (レート制限対応): '{query}' (最大{max_results}件)")
        ...
        try:
            ...
            semantic_result = await self.semantic_scholar.search_papers(query, per_api_results)
    ...
    def _select_best_paper(self, papers: List[Paper]) -> Paper:
        ...
        papers.sort(
            key=lambda p: api_priority.get(
                p.source_api, 0), reverse=True)
