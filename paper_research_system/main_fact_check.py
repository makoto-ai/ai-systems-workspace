...
📊 機能:
  ...
  ✅ YouTube最適化
    ...
    # 引数定義
    parser.add_argument("manuscript", nargs="?", help="事実確認する原稿テキスト")

    parser.add_argument("--file", "-f", type=str, help="原稿ファイルパス")

    parser.add_argument("--interactive", "-i", action="store_true", help="対話モード")

    parser.add_argument("--output", "-o", type=str, help="修正版の保存先ファイル")

    parser.add_argument(
        "--detailed", "-d", action="store_true", help="詳細な分析結果を表示"
    )
...
def display_results(result: dict, args):
    ...
    if args.detailed:
        ...
        for i, fact_result in enumerate(result["fact_check_results"], 1):
            ...
            print(
                f"   ハルシネーション: {'❌' if fact_result.is_hallucination else '✅'}"
            )
