...
@click.option(
    ...
    "-t",
    default="thesis",
    type=click.Choice(["thesis", "antithesis", "synthesis", "meta_analysis"]),
    help="クリティカルシンキングモード",
)
...
def specialized_search(
    ...
    papers, metadata = asyncio.run(
        search_service.specialized_search(
            query,
            domain,
            thinking_mode,
            max_results * 2 if has_filters else max_results,
        )
    )
...
def _display_specialized_format(
    ...
    # 結果テーブル
    table = Table(
        title="🎯 特化検索結果", show_header=True, header_style="bold magenta"
    )
...
def _display_table(papers: List[Paper], recommendations: List = None):
    """通常テーブル形式"""
    table = Table(title="📚 検索結果", show_header=True, header_style="bold magenta")
