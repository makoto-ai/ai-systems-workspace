...
class SystemReliabilityEnforcer:
    ...
    async def _test_integration_system(self) -> SystemTest:
        ...
        try:
            ...
            if all(integration_checks):
                ...
                missing_features = []
                if not result.get('success'): missing_features.append("基本機能")
                if 'research_data' not in result: missing_features.append("論文検索連携")
                if 'verified_info' not in result: missing_features.append("情報検証")
                if 'verification_result' not in result: missing_features.append("品質検証")
    ...
    def _get_recommendation(self, reliability: str) -> str:
        ...
        if reliability == "unreliable":
            return "❌ この発言は控えてください。実動テストを行ってから発言してください。"
        elif reliability == "mixed":
            return "⚠️ 表現を見直してください。より正確な表現に変更することをお勧めします。"
    ...
    def generate_reliability_summary(self) -> str:
        ...
        for system_name, report in self.reliability_reports.items():
            ...
            summary += f"   最終テスト: {report.last_tested.strftime('%Y-%m-%d %H:%M:%S')}\n"
...
if __name__ == "__main__":
    ...
    async def demo():
        ...
        for statement in test_statements:
            ...
            print(f"   「{statement}」 → {check['reliability']} {check['recommendation']}")
