...
MCP統合版 - 最大限の事実正確性とアルゴリズム性能（エラー修正版）
"""
...
try:
    ...
    logging.warning("aiohttp not available, HTTP requests will be disabled")
...
try:
    ...
    logging.warning("openai not available, GPT functionality will be disabled")
...
try:
    ...
    logging.warning("anthropic not available, Claude functionality will be disabled")
...
class MCPConstitutionalAI:
    ...
    async def generate_with_constitution(self, prompt: str, research_data: ResearchMetadata, 
        ...
        if not self.anthropic:
            # フォールバック: モック生成
            logger.warning("Anthropic not available, using mock generation")
            return self._generate_mock_content(prompt, research_data)
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt == max_retries - 1:
                    # 最終リトライ失敗時はモック生成
                    logger.warning("All retries failed, using mock generation")
                    return self._generate_mock_content(prompt, research_data)
    ...
    def _generate_mock_content(self, prompt: str, research_data: ResearchMetadata) -> str:
        """モックコンテンツ生成（フォールバック）"""
        return f"""
{research_data.title}についてのYouTube原稿

この研究では、{research_data.abstract}
...
この研究は、{', '.join(research_data.authors)}によって{research_data.publication_year}年に発表されました。
"""
...
class ModelRouter:
    ...
    async def _use_gpt(self, content: str, research_data: ResearchMetadata) -> str:
        ...
        if not self.openai_client:
            # フォールバック: モック生成
            logger.warning("OpenAI not available, using mock generation")
            return self._generate_mock_gpt_content(content, research_data)
    ...
    def _generate_mock_gpt_content(self, content: str, research_data: ResearchMetadata) -> str:
        """モックGPTコンテンツ生成（フォールバック）"""
        return f"""
{{
    "summary": "{research_data.title}の研究について",
    "key_points": [
        "研究の背景と重要性",
        "具体的な手法と結果",
        "実用的な応用例"
    ...
    "sources": [
        {{
            "type": "academic",
            "url": "https://doi.org/{research_data.doi or 'example'}",
            "doi": "{research_data.doi or 'N/A'}"
        }}
    ],
